...
class LoggingStrategy(bt.Strategy):
    ...
    def __init__(self):
        ...
        if hasattr(self.datas.p, 'dataname') and isinstance(self.datas.p.dataname, pd.DataFrame):
             # 对于PandasData, dataname是DataFrame本身, 可能没有简单的名称属性
             # 可以考虑在加载数据时给数据馈送命名: cerebro.adddata(data_feed, name='MyStock')
             # 然后通过 self.datas.p.name 访问
             print(f"Data Source: Pandas DataFrame (Index: {self.datas.p.dataname.index.name})")
        elif hasattr(self.datas.p, 'dataname'):
             print(f"Data Source: {self.datas.p.dataname}")
