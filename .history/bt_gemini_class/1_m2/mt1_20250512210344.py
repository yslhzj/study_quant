...
class SmaCrossADXFilter(bt.Strategy):
    ...
    def __init__(self):
        ...
        self.adx = bt.indicators.AverageDirectionalMovementIndex(
            self.datas[0], period=self.p.adx_period, plot=False  # 添加plot=False参数不在observer图中显示
        )
    ...
    def next(self):
        ...
        if not self.position:
            ...
            if self.sma_crossover > 0 and current_adx > self.p.adx_threshold:
                self.log(
                    f'BUY CREATE, Close: {self.dataclose[0]:.2f}, ADX: {current_adx:.2f}, Size: {self.getsizing(self.dataclose[0]):.0f} shares')
                self.order = self.buy()
        else:  # 持有仓位
