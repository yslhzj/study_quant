...
class TradeLinesObserver(bt.Observer):
    ...
    lines = ('long_entry', 'long_exit', 'short_entry',
             'short_exit', 'trade_line')
    ...
    plotlines = dict(
        ...
        short_exit=dict(_name='Cover', marker='^', markersize=8, color='yellow',
                        fillstyle='full', ls='', linewidth=1),
        trade_line=dict(_name='Trade', ls='-', linewidth=1,
                        color='gray', _plotskip=False)
    )
    ...
    def next(self):
        ...
        if self.last_entry_price is not None:
            self.lines.trade_line[0] = self.last_entry_price
        ...
        for trade in self._owner._tradespending:
            if trade.isclosed:  # 交易已关闭
                ...
                if trade.pnl > 0:
                    self.lines.long_exit[0] = 1.1  # 盈利平仓标记

                    # 如果有前面记录的入场点，绘制连线
                    if self.last_entry_price is not None:
                        # 在当前位置也设置交易线值，形成连线
                        self.lines.trade_line[0] = 1.1
                        self.last_entry_price = None  # 重置入场点信息
                else:
                    self.lines.short_exit[0] = -1.1  # 亏损平仓标记
            ...
            if trade.justopened:
                ...
                try:
                    ...
                    if (hasattr(trade, 'history') and trade.history and
                        ...
                        self.lines.long_entry[0] = 1.1  # 多头入场标记
                        # 记录入场点信息
                        self.last_entry_price = 1.1
                        self.last_entry_time = len(self)
