...
class TradeEventObserver(bt.Observer):
    ...
    # 定义两条线用于绘制，一条标记盈利交易，一条标记亏损交易
    lines = ('trade_exit_win', 'trade_exit_loss')

    # 绘图设置
    plotinfo = dict(plot=True, subplot=False, plotname='Trade Events')
    plotlines = dict(
        trade_exit_win=dict(marker='o', markersize=8, color='green', ls=''),
        trade_exit_loss=dict(marker='v', markersize=8, color='red', ls='')
    )

    def __init__(self):
        super().__init__()
        # 初始化时不直接设置lines值，避免初始化顺序错误
        # 判断是否能找到策略实例
    ...
    def notify_trade(self, trade):
        ...
        if trade.isclosed:
            ...
            if trade.pnlcomm > 0:  # 盈利交易
                self.lines.trade_exit_win[idx] = trade.price
                self.lines.trade_exit_loss[idx] = float('nan')
            else:  # 亏损交易
                self.lines.trade_exit_win[idx] = float('nan')
                self.lines.trade_exit_loss[idx] = trade.price

    def next(self):
        """在每个bar上设置初始值为nan，表示没有交易关闭"""
        idx = len(self) - 1
        self.lines.trade_exit_win[idx] = float('nan')
        self.lines.trade_exit_loss[idx] = float('nan')
