...
class TradeEventObserver(bt.Observer):
    ...
    # 每条线的绘图属性
    plotlines = dict(
        trade_exit_win=dict(marker='o', markersize=10,
                            color='lime', fillstyle='full', ls=''),
        trade_exit_loss=dict(marker='v', markersize=10,
                             color='red', fillstyle='full', ls='')
    )

    def __init__(self):
        # 初始化所有点为NaN
        self.lines.trade_exit_win = float('nan')
        self.lines.trade_exit_loss = float('nan')
