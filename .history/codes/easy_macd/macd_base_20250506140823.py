...
class TradeEventObserver(bt.Observer):
    ...
    lines = ('trade_exit_win', 'trade_exit_loss')

    # 绘图设置 - 在主图上显示(subplot=False)
    plotinfo = dict(
        plot=True,
        subplot=False,  # 显示在主图上
        plotname='Trade Events'
    )

    # 每条线的绘图属性
    plotlines = dict(
        trade_exit_win=dict(marker='o', markersize=12,
                            color='#00FF00', fillstyle='full', ls=''),
        trade_exit_loss=dict(marker='v', markersize=12,
                             color='#FF0000', fillstyle='full', ls='')
    )

    def __init__(self):
        # 初始化时打印确认消息
        print("TradeEventObserver初始化成功")

    def start(self):
        # Observer开始时的动作
        print("TradeEventObserver启动")

    def prenext(self):
        # 确保在每个周期都设置了NaN，即使是未达到最小周期的情况
        self.lines.trade_exit_win[0] = float('nan')
        self.lines.trade_exit_loss[0] = float('nan')
    ...
    def notify_trade(self, trade):
        ...
        # 打印交易信息以便调试
        print(
            f"交易关闭 - ID: {trade.ref}, 价格: {trade.price}, 盈亏: {trade.pnlcomm}")

        # 根据盈亏情况在当前bar位置标记点位
        price = trade.price
        if trade.pnlcomm > 0:  # 盈利交易
            self.lines.trade_exit_win[0] = price
            print(f"标记盈利点位: {price}")
        else:  # 亏损交易
            self.lines.trade_exit_loss[0] = price
            print(f"标记亏损点位: {price}")

    def stop(self):
        # Observer停止时的动作
        print("TradeEventObserver停止")
...
if __name__ == '__main__':
    ...
    cerebro.addobserver(TradeEventObserver)  # 添加新的交易事件观察器
    ...
    if len(results_df) > 0:
        ...
        cerebro_best.addobserver(TradeEventObserver)  # 添加新的交易事件观察器
