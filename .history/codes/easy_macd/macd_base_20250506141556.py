...
class TradeEventObserver(bt.Observer):
    """交易事件观察器，在交易关闭时在主图上标记盈亏点位"""

    # 定义两条线用于绘制标记点
    lines = ('trade_win', 'trade_loss')
    ...
    # 每条线的绘图属性
    plotlines = dict(
        trade_win=dict(marker='o', markersize=8,
                       color='lime', fillstyle='full', ls=''),
        trade_loss=dict(marker='v', markersize=8,
                        color='red', fillstyle='full', ls='')
    )

    def __init__(self):
        # 调用父类初始化
        super(TradeEventObserver, self).__init__()
        # 初始化lines
        for line in self.lines:
            line.plotvalue = True  # 确保值被绘制

        print("TradeEventObserver初始化完成")

    def next(self):
        # 每个周期默认不显示标记点
        self.lines.trade_win[0] = float('nan')
    ...
    def notify_trade(self, trade):
        ...
        print(f"标记交易: ID={trade.ref}, 价格={trade.price}, PnL={trade.pnlcomm}")
        ...
        if trade.pnlcomm > 0:
            # 盈利交易用绿色圆点标记
            self.lines.trade_win[0] = price
            print(f"标记盈利交易: 价格={price}")
        else:
            # 亏损交易用红色三角形标记
            self.lines.trade_loss[0] = price
            print(f"标记亏损交易: 价格={price}")
...
class TestStrategy(bt.Strategy):
    ...
    def __init__(self):
        ...
        # 添加交易信息记录
        self.trade_entry_info = {}  # 记录开仓信息
        self.trade_exit_info = {}   # 记录平仓信息
    ...
    def notify_trade(self, trade):
        ...
        if trade.justopened:
            ...
            print(
                f'===== 交易开始: #{trade.ref}, 价格={trade.price}, 大小={trade.size} =====')
        ...
        if trade.isclosed:
            ...
            print(f'===== 交易关闭: #{trade.ref} =====')
            print(f'价格: {trade.price}')
            print(f'毛利: {trade.pnl:.2f}, 净利: {trade.pnlcomm:.2f}')
            print(f'开仓: {self.trade_entry_info[trade.ref]["datetime"]}')
            print(f'平仓: {self.datetime.datetime(0)}')
...
if __name__ == '__main__':
    ...
    if len(results_df) > 0:
        ...
        # 创建新的Cerebro实例用于单次回测，并启用默认统计图
        cerebro_best = bt.Cerebro(stdstats=True)
        ...
        # 在最终验证回测中添加交易事件观察器
        print("添加交易事件观察器到最终验证回测...")
        cerebro_best.addobserver(TradeEventObserver)
        ...
        # 绘制图表
        print("\n正在生成回测图表...")
        cerebro_best.plot(style='candlestick', barup='red', bardown='green')
    else:
