...
if __name__ == '__main__':
    ...
    cerebro = bt.Cerebro(stdstats=False, optreturn=True, maxcpus=1)  # 设置 optreturn=True, maxcpus=1使优化过程更易于观察
    ...
    for i, result in enumerate(results):
        ...
        if i % 40 == 0 or i == len(results) - 1:
            print(f"正在处理优化结果: {i+1}/{len(results)} ({(i+1)/len(results)*100:.1f}%)")
    ...
    print(f"处理完成! 有效参数组合数: {valid_count}/{len(results)} ({valid_count/len(results)*100:.1f}%)")
    print("="*50)
    ...
    if len(results_df) > 0:
        ...
        print("\n" + "=" * 15 + " 优化结果统计 " + "=" * 15)
        print("各指标的统计描述:")
        print(results_df[['net_profit', 'max_drawdown', 'sharpe', 'sqn', 'profit_factor', 'trades']].describe())
        print("="*50)
        ...
        for fast, count in fast_counts.items():
            print(f"  - fast_ema={fast}: {count}次 ({count/len(results_df)*100:.1f}%)")
            
        print("\nslow_ema分布:")
        for slow, count in slow_counts.items():
            print(f"  - slow_ema={slow}: {count}次 ({count/len(results_df)*100:.1f}%)")
            
        print("\nsignal_ema分布:")
        for signal, count in signal_counts.items():
            print(f"  - signal_ema={signal}: {count}次 ({count/len(results_df)*100:.1f}%)")
        ...
        print(f"综合得分: {best_profit['score']:.4f}, 排名: {results_df['score'].rank(ascending=False)[best_profit.name]}/{len(results_df)}")
        ...
        print(f"综合得分: {best_drawdown['score']:.4f}, 排名: {results_df['score'].rank(ascending=False)[best_drawdown.name]}/{len(results_df)}")
        ...
        try:
            ...
            total_trades = ta_val.total.closed if hasattr(ta_val, 'total') else 0
            ...
            print(f"获利交易: {win_trades}, 亏损交易: {loss_trades}, 胜率: {win_rate:.2f}%")
