...
class TradeEventObserver(bt.Observer):
    ...
    def notify_trade(self, trade):
        if trade.isclosed:
            ...
            if entry_bar is not None and entry_price is not None:
                # 将开仓点信息添加到待标记列表中
                self.pending_entry_marks.append({
                    'bar': entry_bar,
                    'price': entry_price
                })
    ...
    def next(self):
        ...
        # 检查并处理待标记列表中的开仓点
        current_idx = len(self) - 1  # 当前bar的索引

        # 遍历待标记列表，标记当前bar对应的开仓点
        i = 0
        while i < len(self.pending_entry_marks):
            mark = self.pending_entry_marks[i]
            if mark['bar'] == current_idx:
                # 找到当前bar对应的开仓点，进行标记
                self.lines.trade_entry[0] = mark['price']
                # 标记完成后移除
                self.pending_entry_marks.pop(i)
            elif mark['bar'] < current_idx:
                # 如果开仓点的bar索引小于当前索引，说明已经错过了标记时机，直接移除
                self.pending_entry_marks.pop(i)
            else:
                # 开仓点的bar索引大于当前索引，保留等待后续处理
                i += 1
