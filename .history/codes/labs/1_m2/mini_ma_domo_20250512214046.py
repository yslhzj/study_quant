...
class SmaCrossADXFilter(bt.Strategy):
    ...
    params = (
        ('fast_ma_period', 12),   # 优化快线周期，从10改为12
        ('slow_ma_period', 26),   # 优化慢线周期，从30改为26
        ('adx_period', 14),
        ('adx_threshold', 20.0),  # 降低ADX阈值以捕获更多交易机会
        ('profit_take', 0.05),    # 新增止盈参数，5%止盈
        ('stop_loss', 0.03),      # 新增止损参数，3%止损
    )
    ...
    def __init__(self):
        ...
        print(
            f"Stop Loss: {self.p.stop_loss*100}%, Profit Take: {self.p.profit_take*100}%")
    ...
    def notify_order(self, order):
        ...
        if order.status in [bt.Order.Completed]:
            if order.isbuy():
                self.entry_price = order.executed.price  # 记录买入价格
                self.log(
    ...
    def next(self):
        ...
        if not self.position:
            ...
            if self.entry_price is not None:
                ...
                # 止损条件
                if price_change < -self.p.stop_loss:
                    self.log(
                        f'STOP LOSS, Close: {self.dataclose[0]:.2f}, Price Change: {price_change*100:.2f}%')
                    self.order = self.close()
                    return

                # 止盈条件
                if price_change > self.p.profit_take:
                    self.log(
                        f'PROFIT TAKE, Close: {self.dataclose[0]:.2f}, Price Change: {price_change*100:.2f}%')
                    self.order = self.close()
                    return

            # 卖出信号：快线下穿慢线 且 ADX低于阈值(增加ADX退场过滤)
            if self.sma_crossover < 0 and current_adx < self.p.adx_threshold * 0.8:
                self.log(
                    f'SELL CREATE (Cross Down + ADX Filter), Close: {self.dataclose[0]:.2f}, ADX: {current_adx:.2f}')
                self.order = self.close()
            # 卖出信号：快线下穿慢线 (保留原有条件作为备选)
            elif self.sma_crossover < 0:
                self.log(
                    f'SELL CREATE (Cross Down), Close: {self.dataclose[0]:.2f}')
                self.order = self.close()
