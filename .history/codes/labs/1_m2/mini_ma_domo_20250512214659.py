...
class SharpeRatioWithDrawdown(bt.Analyzer):
    ...
    def create_analyzers(self):
        # 创建所需的分析器并添加到策略中
        self.ret_a = self.strategy.analyzers.add(bt.analyzers.Returns)
        self.sharpe_a = self.strategy.analyzers.add(bt.analyzers.SharpeRatio_A)
        self.drawdown_a = self.strategy.analyzers.add(bt.analyzers.DrawDown)
    ...
    def get_analysis(self):
        # 获取其他分析器的结果并计算综合得分
        ret = self.ret_a.get_analysis().get('rtot', 0.0)
        sharpe = self.sharpe_a.get_analysis().get('sharperatio', 0.0)
        dd = self.drawdown_a.get_analysis().get('max', {}).get('drawdown', 0.0)

        return {
            'returns': ret,
            'sharpe': sharpe,
            'drawdown': dd,
            'score': sharpe - dd * 0.5  # 综合评分：夏普比率减去加权回撤
        }
...
if __name__ == '__main__':
    ...
    cerebro.optstrategy(
        SmaCrossADXFilter,
        fast_ma_period=[5, 10, 15, 20],         # 更精简的搜索范围
        slow_ma_period=[20, 30, 40, 50, 60],    # 同上保持不变
        adx_period=[10, 14, 18],                # 更精简的搜索范围
        adx_threshold=[20, 25, 30, 35]          # 更合理的ADX阈值范围
    )
