...
class SharpeRatioWithDrawdown(bt.Analyzer):
    ...
    def get_analysis(self):
        ...
        # 获取各项指标，如果分析器不存在则使用默认值
        ret = ret_analyzer.get_analysis().get('rtot', 0.0) if ret_analyzer else 0.0
        sharpe = sharpe_analyzer.get_analysis().get(
            'sharperatio', 0.0) if sharpe_analyzer else 0.0
        dd = dd_analyzer.get_analysis().get('max', {}).get(
            'drawdown', 0.0) if dd_analyzer else 0.0
...
if __name__ == '__main__':
    ...
    if best_params:
        ...
        cerebro_best.addanalyzer(bt.analyzers.SharpeRatio_A, _name='sharpe')
        ...
        print(f'回测收益率: {strat.analyzers.returns.get_analysis()["rtot"]:.2%}')
        print(
            f'回测夏普比率: {strat.analyzers.sharpe.get_analysis()["sharperatio"]:.2f}')
        print(
            f'回测最大回撤: {strat.analyzers.drawdown.get_analysis()["max"]["drawdown"]:.2%}')
