...
if __name__ == '__main__':
    ...
    # 禁用多进程可能会更稳定，尤其是在遇到错误时
    cerebro.run(maxcpus=1)  # 使用单进程运行，避免多进程问题
    ...
    try:
        ...
        if best_params:
            ...
            try:
                ...
                print(f'回测收益率: {ret_analysis.get("rtot", 0.0):.2%}')

                sharpe_analysis = strat.analyzers.sharpe.get_analysis()
                print(f'回测夏普比率: {sharpe_analysis.get("sharperatio", 0.0):.2f}')

                dd_analysis = strat.analyzers.drawdown.get_analysis()
                max_dd = dd_analysis.get('max', {})
                print(f'回测最大回撤: {max_dd.get("drawdown", 0.0):.2%}')
            except Exception as e:
