...
class SmaCrossADXFilter(bt.Strategy):
    ...
    params = (
        ...
        ('verbose', False),      # 控制是否打印详细日志
    )

    def log(self, txt, dt=None, is_init=False):
        # 仅在verbose=True或初始化时打印日志
        if self.p.verbose or is_init:
    ...
    def __init__(self):
        ...
        self.log(
            "--- SMA Crossover ADX Filter Strategy Initialized ---", is_init=True)
        self.log(
            f"Fast MA: {self.p.fast_ma_period}, Slow MA: {self.p.slow_ma_period}, ADX Period: {self.p.adx_period}, ADX Threshold: {self.p.adx_threshold}", is_init=True)
...
def opt_callback(result):
    ...
    if sharpe > 1.0 and max_dd < 0.20 and total_return > 10.0:
        print('-' * 50)
        print(f'【优质策略参数】Fast MA: {params.fast_ma_period}, Slow MA: {params.slow_ma_period}, '
...
if __name__ == '__main__':
    ...
    cerebro.optstrategy(
        ...
        verbose=False                         # 优化过程中不打印详细日志
    )

    print(f'初始投资组合价值: {cerebro.broker.getvalue():.2f} RMB')
    results = cerebro.run()
    print(f'优化完成，总共测试了 {len(results)} 组参数')
    print('=' * 50)
    ...
    if best_params:
        ...
        returns = best_result.analyzers.returns.get_analysis()
        total_return = returns.get('rtot', 0.0)
