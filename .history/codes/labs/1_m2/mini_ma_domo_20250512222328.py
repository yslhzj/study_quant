...
if __name__ == '__main__':
    ...
    # --- 添加分析器 ---
    # 为了获取收益和回撤等指标，我们需要添加相应的分析器
    cerebro.addanalyzer(bt.analyzers.SharpeRatio_A,
                        _name='sharpe', timeframe=bt.TimeFrame.Years)  # 年化夏普比率
    ...
    for run_result in opt_results:
        # opt_results 中的每个元素是一个包含单个策略实例的列表
        strategy_instance = run_result[0]

        # 获取参数
        params = strategy_instance.params.__dict__.copy()  # 使用 .copy() 来获取参数字典

        # 获取分析器结果
        # 夏普比率, 注意处理未产生交易或收益为0导致sharperatio为None的情况
        sharpe_analysis = strategy_instance.analyzers.sharpe.get_analysis()
        sharpe_ratio = sharpe_analysis.get(
            'sharperatio', float('-inf'))  # 如果无法计算，默认为负无穷大
        if sharpe_ratio is None:  # 再次确保sharpe_ratio是浮点数
            sharpe_ratio = float('-inf')

        # 最大回撤
        drawdown_analysis = strategy_instance.analyzers.drawdown.get_analysis()
        max_drawdown_money = drawdown_analysis.max.moneydown  # 金额回撤
        max_drawdown_percent = drawdown_analysis.max.drawdown  # 百分比回撤
    ...
    if not sorted_results:
        ...
        for i, result in enumerate(sorted_results[:5]):  # 打印前5个结果
            print(f"Rank {i+1}:")
            # 为了更清晰地展示参数，我们直接从result['params']中提取
            p = result['params']
            print(
    ...
    # 如果夏普比率普遍很低或无法计算，给出提示
    if sorted_results and sorted_results[0]['sharpe_ratio'] == float('-inf'):
        print("\nWarning: Sharpe Ratio could not be calculated for the top strategies.")
        print("This might happen if there were no trades or returns were consistently zero/negative.")
        print("Consider reviewing your data, strategy logic, or parameter ranges.")

    # cerebro.plot() # 优化后通常不直接绘图，而是分析数据
