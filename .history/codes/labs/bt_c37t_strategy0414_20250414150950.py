...
class AShareETFStrategy(bt.Strategy):
    ...
    params = (
        ...
        ('log_filename_pattern',
         '{strategy_name}_{timestamp:%Y%m%d_%H%M%S}_{pid}.jsonl'),
        ('log_level', 'INFO'),
    ...
    def log_event(self, event_type: str, data_feed=None, **kwargs):
        ...
        try:
            ...
            # 确定要使用的数据源
            _data = data_feed if data_feed is not None else (
                self.datas[0] if self.datas else None)
            data_id = _data._name if _data and hasattr(
                _data, '_name') else "GLOBAL"
    ...
    def _setup_logger(self):
        ...
        try:
            ...
            self._logger = logging.getLogger(
                f"{strategy_name}_{self.strategy_id}")
    ...
    def notify_cashvalue(self, cash, value):
        ...
        # 记录资金变动事件
        self.log_event(
            self.EVENT_CASHVALUE_UPDATE if hasattr(
                self, 'EVENT_CASHVALUE_UPDATE') else 'CASHVALUE_UPDATE',
            None,
            cash=cash,
            value=value,
    ...
    def _calculate_trade_size(self, data_close_price, entry_price, stop_loss_price, risk_per_trade_percent, data=None):
        ...
        if potential_trade_value > max_pos_value:
            ...
            adjustment_reasons.append(
                f"MAX_POSITION_LIMIT: {size} -> {new_size}")
            size = new_size
            # 更新仓位大小。 (更新买入股数。)
