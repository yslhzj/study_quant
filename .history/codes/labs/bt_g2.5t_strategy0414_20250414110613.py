...
class AShareETFStrategy(bt.Strategy):
    ...
    params = (
        ...
        ('log_filename_pattern', '{strategy_name}_{data_name}_{timestamp}_{pid}.log'), # 日志文件名模板 (Log filename template)
        ('log_level', 'INFO'), # 日志级别 (Log level)
        ('log_async_write', True), # 是否异步写入日志 (Whether to write logs asynchronously)
        # ------------------------
    )
    ...
    def __init__(self, **kwargs):  # 接受 **kwargs 以兼容 addstrategy 传递的参数
        ...
        # (初始化日志系统)
        self.strategy_id = f"{self.__class__.__name__}_{uuid.uuid4().hex[:8]}" # 创建唯一的策略实例 ID (Create a unique strategy instance ID)
        ...
        try:
            ...
            print(f"Error setting up logger for strategy {self.strategy_id}: {e}", file=sys.stderr)
            self.logger = logging.getLogger('fallback_logger') # 提供一个备用 logger (Provide a fallback logger)
            self.logger.addHandler(logging.NullHandler()) # 避免 "No handler found" 警告 (Avoid "No handler found" warnings)

        # 初始化交易周期 ID 和订单映射 (Initialize trade cycle ID and order mapping)
        self.current_trade_id = None # 当前活跃的交易周期 ID (Current active trade cycle ID)
        self.order_ref_to_trade_id_map = {} # 订单引用到交易周期 ID 的映射 (Mapping from order reference to trade cycle ID)
