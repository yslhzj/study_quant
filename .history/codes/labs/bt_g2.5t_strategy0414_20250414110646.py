...
class AShareETFStrategy(bt.Strategy):
    ...
    def __init__(self, **kwargs):  # 接受 **kwargs 以兼容 addstrategy 传递的参数
        ...
        self.log_event(
            ...
            data_ids=[d._name for d in self.datas] # 获取所有数据源的名称 (Get names of all data feeds)
        )
        ...
        self.ema_shorts = [bt.indicators.ExponentialMovingAverage(d, period=self.p.ema_short_period) for d in self.datas]
        # 计算短期EMA (Calculate short-term EMA)
        self.ema_mediums = [bt.indicators.ExponentialMovingAverage(d, period=self.p.ema_medium_period) for d in self.datas]
        # 计算中期EMA (Calculate medium-term EMA)
        self.ema_longs = [bt.indicators.ExponentialMovingAverage(d, period=self.p.ema_long_period) for d in self.datas]
        # 计算长期EMA (Calculate long-term EMA)
        self.rsis = [bt.indicators.RelativeStrengthIndex(d, period=self.p.rsi_period) for d in self.datas]
        # 计算RSI (Calculate RSI)
        self.bbands = [bt.indicators.BollingerBands(d, period=self.p.bbands_period, devfactor=self.p.bbands_devfactor) for d in self.datas]
        # 计算布林带 (Calculate Bollinger Bands)
        self.atrs = [bt.indicators.AverageTrueRange(d, period=self.p.atr_period) for d in self.datas]
        # 计算ATR (Calculate ATR)

        # 状态变量
        # (State Variables)
        self.order = None # 用于跟踪当前活动的订单 (Used to track the currently active order)
        self.trade_count = 0 # 交易计数 (Trade count)
        self.trailing_stop_price = None # 跟踪止损价格 (Trailing stop price)
        self.high_water_mark = self.broker.getvalue() # 账户最高价值，用于计算回撤 (Highest account value for drawdown calculation)
        self.drawdown = 0.0 # 当前回撤百分比 (Current drawdown percentage)
        self.in_trade_halt = False # 是否处于交易暂停状态 (Whether in trade halt status)
        self.active_orders = {} # 存储活动订单引用及其类型 {order_ref: type} (Stores active order references and their types)
        self.partial_exit_executed = False # 标记部分退出是否已执行 (Flag if partial exit has been executed)

    def log_event(self, event_type: str, data_feed: bt.DataBase, **kwargs):
        """统一的日志记录接口"""
        # (The unified logging interface)
        if not self.logger: # 如果 logger 未初始化，则不记录 (If logger is not initialized, do not log)
            return
        ...
        try:
            ...
            log_entry = {
                ...
                "data_id": data_feed._name if data_feed else 'N/A', # 获取数据源名称 (Get data feed name)
            }
            ...
            # 如果是订单状态更新或交易关闭事件，尝试从映射中查找 trade_cycle_id (If it's an order status update or trade closed event, try to find trade_cycle_id from the map)
                ...
                if isinstance(order_ref, bt.Order):
                    order_ref = order_ref.ref # 如果传入的是 Order 对象，获取其 ref (If an Order object is passed, get its ref)
                log_entry['trade_cycle_id'] = self.order_ref_to_trade_id_map.get(order_ref, 'ORPHANED_EVENT') # 从映射获取，记录孤立事件 (Get from map, log orphaned events)
            elif event_type == 'TRADE_CLOSED' and 'trade' in kwargs:
                trade = kwargs['trade']
                found_trade_id = 'ORPHANED_TRADE' # 默认是孤立交易 (Default to orphaned trade)
                if hasattr(trade, 'orders') and trade.orders:
                    for order_ref in trade.orders:
                        mapped_id = self.order_ref_to_trade_id_map.get(order_ref)
                        if mapped_id:
                            found_trade_id = mapped_id
                            break # 找到第一个匹配的即可 (Break once the first match is found)
            ...
            self.logger.info(log_entry) # 使用 .info() 并传入字典 (Use .info() and pass the dictionary)

        except Exception as e:
             # 打印日志记录期间发生的任何错误 (Print any errors that occur during logging)
            print(f"Error during logging event {event_type}: {e}", file=sys.stderr)
    ...
    def start(self):
        ...
        self.log_event(event_type="STRATEGY_START", data_feed=self.datas[0], initial_value=self.broker.getvalue())
