...
class AShareETFStrategy(bt.Strategy):
    ...
    def notify_order(self, order):
        ...
        data = order.data # 获取订单关联的数据源 (Get data feed associated with the order)
        status_name = order.getstatusname() # 获取订单状态名称 (Get order status name)

        # --- Log Order Status Update ---
        # (记录订单状态更新事件)
        exec_details = None # 初始化执行详情为 None (Initialize execution details to None)
        remaining_size = None # 初始化剩余大小为 None (Initialize remaining size to None)
        if order.executed:
            ...
            remaining_size = order.executed.remsize # 获取剩余未执行的大小 (Get the remaining unexecuted size)
        ...
        self.log_event(
            ...
            parent_ref=order.parent.ref if order.parent else None, # 记录父订单引用 (Log parent order reference)
            oco_ref=order.oco.ref if order.oco else None # 记录 OCO 订单引用 (Log OCO order reference)
        )
        # --- End Log ---
        ...
        if order.status in [order.Submitted, order.Accepted]:
            ...
            self.active_orders[order_ref] = order.ordtypename() # 记录活动订单 (Record active order)
            return

        if order.status in [order.Completed]:
            # 如果订单已完成 (If the order is completed)
            if order_ref in self.active_orders:
                del self.active_orders[order_ref] # 从活动订单中移除 (Remove from active orders)
            if order.isbuy():
                # 如果是买入订单完成。
                # (If it is a buy order completed.)
                self.log(f'{data._name} BUY EXECUTED, Price: {order.executed.price:.2f}, Cost: {order.executed.value:.2f}, Comm {order.executed.comm:.2f}', data=data)
                ...
                self.log(f'{data._name} SELL EXECUTED, Price: {order.executed.price:.2f}, Cost: {order.executed.value:.2f}, Comm {order.executed.comm:.2f}', data=data)
            ...
            self.log(f'{data._name} Order {order.getstatusname()}: Ref:{order.ref}', data=data)
            # 记录订单状态。
            ...
            is_bracket = order.parent or order.oco # 检查是否是 Bracket 订单的一部分 (Check if part of a Bracket order)
            if not is_bracket and order_ref in self.order_ref_to_trade_id_map:
                ...
                try:
                    ...
                    self.log_event(event_type="MAP_CLEANUP", data_feed=data, reason="Order Final State (Non-Bracket)", order_ref=order_ref)
                except KeyError:
                    pass # 可能已被 notify_trade 清理 (May have already been cleaned by notify_trade)
            # --- End Cleanup ---
