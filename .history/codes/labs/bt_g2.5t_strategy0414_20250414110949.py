...
class AShareETFStrategy(bt.Strategy):
    ...
    def next(self):
        ...
        self.log(f'Next Start at {current_dt}')
        ...
        if self.in_trade_halt:
            self.log("Trading is halted due to prior drawdown breach.")
            # 如果处于交易暂停状态，记录日志并跳过本周期。 (If trading is halted, log and skip this cycle.)
            # 检查是否有需要平仓的逻辑（例如，时间到了或特定条件满足）
            # Check if there's logic to close positions (e.g., time limit or specific conditions)
            # ... (平仓逻辑)
            return

        # 检查是否有活动订单，防止重复下单 (Check for active orders to prevent duplicate orders)
        if self.order:
            self.log(f"Active order {self.order.ref} exists, skipping new order placement.")
            return
            ...
            if position:
                ...
                if self.p.use_partial_exit and not self.partial_exit_executed and position.size > 0:
                    ...
                    if close_price >= partial_exit_target:
                        ...
                        partial_size = int(position.size * self.p.partial_exit_fraction / 100) * 100 # 计算部分退出数量 (Calculate partial exit size)
                        if partial_size > 0:
                            self.log(f"{data_name} Triggering Partial Exit at {close_price:.2f}, Target: {partial_exit_target:.2f}, Size: {partial_size}")
                            # --- Log SL/TP Triggered ---
                            # (记录止盈止损触发事件)
                            self.log_event(
                            ...
                            # --- End Log ---
                            order = self.sell(data=d, size=partial_size) # 执行部分卖出 (Execute partial sell)
                            if order:
                                self.partial_exit_executed = True # 标记部分退出已执行 (Mark partial exit as executed)
                                ...
                                self.log(f"{data_name} Failed to submit partial exit order.")
                ...
                if self.p.use_trailing_stop and position.size > 0:
                    ...
                    if self.trailing_stop_price is None:
                        ...
                        initial_stop_loss = position.price - self.p.stop_loss_atr_multiplier * atr # 假设初始止损 (Assume initial stop loss)
                        self.trailing_stop_price = max(initial_stop_loss, stop_price_candidate)
                        ...
                        self.trailing_stop_price = max(self.trailing_stop_price, stop_price_candidate)

                    if close_price <= self.trailing_stop_price:
                        # 如果价格触及移动止损 (If price hits the trailing stop)
                        self.log(f"{data_name} Closing position due to Trailing Stop at {close_price:.2f}, Stop: {self.trailing_stop_price:.2f}")
                        # --- Log SL/TP Triggered ---
                        self.log_event(
                            event_type='SL_TP_TRIGGERED',
                            data_feed=d,
                        ...
                        if self.order:
                            ...
                            self.log(f"{data_name} Failed to submit close order for trailing stop.")
                            ...
                            # --- End Log ---
                        return # 处理完一个退出后跳出当前数据循环 (Exit current data loop after handling an exit)
                ...
                # (常规止损/止盈检查 - 如果移动止损优先触发则冗余)
                    ...
                    if position.size > 0: # 多头止损止盈 (Long SL/TP)
                        if close_price <= stop_loss_price:
                            self.log(f"{data_name} Closing position due to Stop Loss at {close_price:.2f}, Stop: {stop_loss_price:.2f}")
                            # --- Log SL/TP Triggered ---
                            self.log_event(event_type='SL_TP_TRIGGERED', data_feed=d, trigger_type='STOP_LOSS', current_price=close_price, trigger_level=stop_loss_price, position_size=position.size)
                            # --- End Log ---
                            self.order = self.close(data=d)
                        elif close_price >= take_profit_price:
                            self.log(f"{data_name} Closing position due to Take Profit at {close_price:.2f}, Target: {take_profit_price:.2f}")
                             # --- Log SL/TP Triggered ---
                            self.log_event(event_type='SL_TP_TRIGGERED', data_feed=d, trigger_type='TAKE_PROFIT', current_price=close_price, trigger_level=take_profit_price, position_size=position.size)
                            # --- End Log ---
                            self.order = self.close(data=d)
                        ...
                        if self.order:
                            if self.current_trade_id:
                                ...
                                self.log(f"{data_name} Failed to submit close order for SL/TP.")
                                # --- Log Trade Skipped (for the close order) ---
                                self.log_event(
                                    event_type='TRADE_SKIPPED',
                                    data_feed=d,
            ...
            # (入场逻辑)
                ...
                if market_state == 'TREND_UP':
                    ...
                    if ema_short > ema_medium and rsi > 50:
                        ...
                        triggering_data = {'condition': 'ema_short > ema_medium and rsi > 50', 'ema_short': ema_short, 'ema_medium': ema_medium, 'rsi': rsi}
                    ...
                    if close_price <= bb_bot and rsi < 30:
                        ...
                        triggering_data = {'condition': 'close <= bb_bot and rsi < 30', 'close': close_price, 'bb_bot': bb_bot, 'rsi': rsi}
                ...
                if entry_signal:
                    ...
                    # (计算订单详情)
                    entry_price = close_price # 使用收盘价作为近似入场价 (Use close price as approximate entry price)
                    ...
                    size = self._calculate_trade_size(close_price, entry_price, stop_loss_price, risk_per_trade_percent, data=d)
                    ...
                    self.log_event(
                        ...
                        size_raw=size, # 假设 _calculate_trade_size 返回最终调整前的大小 (Assume _calculate_trade_size returns size before final adjustments)
                        size_final=size, # 最终大小 (Final size)
                        adjustment_reasons=[] # 假设 _calculate_trade_size 内部处理了调整原因 (Assume _calculate_trade_size handles adjustment reasons internally)
                    )
                    # --- End Log ---
                    ...
                    # (如果大小 > 0，则下单)
                    if size > 0:
                        self.log(f"{data_name} {entry_signal} Signal. Calculated Size: {size}, Entry: {entry_price:.2f}, SL: {stop_loss_price:.2f}, TP: {take_profit_price:.2f}")
                        # --- Generate Trade Cycle ID ---
                        # (生成交易周期 ID)
                        self.current_trade_id = f"TRADE_{uuid.uuid4().hex[:12]}"
                        self.log_event(event_type="TRADE_CYCLE_START", data_feed=d, trade_cycle_id=self.current_trade_id)
                        # --- End Generate ---
                        ...
                        if entry_signal == 'BUY':
                            ...
                            orders = self.buy_bracket(
                                data=d,
                                size=size,
                                price=entry_price, # 可以用 None 让市价成交，或指定限价 (Can use None for market order or specify limit price)
                                exectype=bt.Order.Market, # 或者 bt.Order.Limit (Or bt.Order.Limit)
                                stopprice=stop_loss_price,
                                stopexec=bt.Order.Stop,
                                limitprice=take_profit_price,
                                limitexec=bt.Order.Limit
                            )
                            if orders and orders[0]: # 检查主订单是否成功创建 (Check if main order was created successfully)
                                    ...
                                    if o:
                                        ...
                                        self.log_event(
                                            ...
                                            order_details={
                                                ...
                                                'is_bracket_component': True # 标记为括号订单组件 (Mark as bracket component)
                                            }
                                        )
                                # --- End Log & Map Update ---
                            else:
                                self.log(f"{data_name} Failed to submit BUY bracket order.")
                                # --- Log Trade Skipped --- 
                                self.log_event(
                                    event_type='TRADE_SKIPPED',
                                    data_feed=d,
                                    reason='Failed to submit BUY bracket order',
                                    details={'intended_size': size, 'signal': signal_type}
                                )
        print("--- 开始单次回测 ---")
        # 添加单次回测策略
