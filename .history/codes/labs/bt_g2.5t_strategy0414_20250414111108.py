...
class AShareETFStrategy(bt.Strategy):
    ...
    def next(self):
        ...
        if self.order:
            # self.log(f"Active order {self.order.ref} exists, skipping new order placement.") # 减少冗余日志 (Reduce redundant logging)
            return
        ...
        for i, d in enumerate(self.datas):
            ...
            # 获取当前成交量 (Get the current volume)
            ...
            self.log_event(
                ...
                indicators={
                 ...
                 position_size=position.size, # 添加当前仓位大小 (Add current position size)
                 cash=self.broker.get_cash(), # 添加当前现金 (Add current cash)
                 value=self.broker.get_value() # 添加当前总价值 (Add current total value)
            )
            # --- End Log ---
