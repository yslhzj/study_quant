...
class AShareETFStrategy(bt.Strategy):
    ...
    def _calculate_trade_size(self, data_close_price, entry_price, stop_loss_price, risk_per_trade_percent, data=None):
        ...
        if stop_loss_price >= entry_price:
            ...
            self.log(f"{data_name}: {reason}. Cannot calculate size.", data=data)
        ...
        size = int(size_raw / 100) * 100 # A股向下取整100股 (Round down to nearest 100 for A-shares)
        raw_size_calculated = size # 记录风险计算后的原始大小 (Record raw size after risk calculation)
        ...
        potential_trade_value = size * current_price_for_calc # 使用调整后的 size (Use adjusted size)
        if potential_trade_value > cash:
            original_size = size
            size = int(cash / current_price_for_calc / 100) * 100
            if size < original_size: # 仅当现金限制导致实际减少时记录 (Only log if cash limit caused actual reduction)
        ...
        for i, d in enumerate(self.datas):
            ...
            # If no position, check for entry conditions
                ...
                if entry_signal and stop_loss_price_calc is not None and entry_price_calc > stop_loss_price_calc:
                    ...
                    size_info = self._calculate_trade_size(
                    ...
                    if size_info['size'] > 0:
                        ...
                        self.log(
                            f'BUY SIGNAL (Bracket): {data_name}, Size: {size_info["size"]}, Entry: ~{entry_price_calc:.2f}, SL: {stop_loss_price_calc:.2f}, TP: {take_profit_price_calc if take_profit_price_calc else "N/A"}, Type: {potential_position_type}', data=d)
    ...
    def get_total_risk_exposure(self):
        ...
        for data in self.datas:
            ...
            if position:
                ...
                pass # 简化示例，需要实现完整的止损跟踪逻辑 (Simplified example, requires full stop-loss tracking logic)
        return total_risk
