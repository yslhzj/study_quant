...
class AShareETFStrategy(bt.Strategy):
    ...
    def notify_order(self, order):
        ...
        if order.status in [order.Submitted, order.Accepted]:
            ...
            self.logger.info(
                f'Order {order.ref} Submitted/Accepted', extra={'data': order_data})
        ...
        self.logger.info(EVENT_TYPES["ORDER_STATUS_UPDATE"], extra=log_kwargs)
        # --- 日志记录结束 ---
...
def get_backtest_config():
    ...
    config = {
        ...
        # (Open interest column name - Set to None as data likely lacks it)
