...
class JsonFormatter(logging.Formatter):
    ...
    def format(self, record):
        ...
        try:
            ...
            # 复用 Encoder
            return json.dumps(log_entry, ensure_ascii=False, cls=BacktraderObjectEncoder)
...
class AShareETFStrategy(bt.Strategy):
    ...
    def log(self, txt, dt=None, level='DEBUG', event_type="GENERAL", extra_data=None, data=None):
        ...
        if not hasattr(self, 'logger') or not self.logger:
            print(
                f"Warning: Logger not initialized for strategy instance {self.instance_id}. Log message: {txt}")
            return
