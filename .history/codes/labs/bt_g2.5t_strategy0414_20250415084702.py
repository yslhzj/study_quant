...
class JsonFormatter(logging.Formatter):
    ...
    def format(self, record):
        ...
        # 如果msg是字典，直接使用；否则包装为字典
        # If msg is a dict, use it directly; otherwise, wrap it in a dict
...
class AShareETFStrategy(bt.Strategy):
    ...
    def _calculate_trade_size(self, data_close_price, entry_price, stop_loss_price, risk_per_trade_percent, data=None, trade_id=None):
        ...
        trade_id = trade_id or self.current_trade_id
        if not trade_id:
            # 如果未提供交易ID并且当前也没有，则尝试生成新的
            self.log("WARNING: No trade_cycle_id provided to _calculate_trade_size", level='WARNING', data=data)
            return 0
        
        # 获取当前账户总价值和每股风险金额
        # Get current portfolio value and risk amount per share
            ...
            # If no position, check for entry conditions
                ...
                if entry_signal and stop_loss_price_calc is not None and entry_price_calc > stop_loss_price_calc:
                    ...
                    if size > 0:
                        ...
                        # 记录订单与交易周期ID的映射关系
                        if bracket_orders:
                            self._record_order_trade_id_mapping(bracket_orders)
    ...
    def _record_order_trade_id_mapping(self, order):
        ...
        if self.current_trade_id is not None:
            ...
            is_child_order = hasattr(
                order, 'parent') and order.parent is not None
