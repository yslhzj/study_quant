...
class AShareETFStrategy(bt.Strategy):
        ...
        if order.status in [order.Completed]:
            ...
            if order.isbuy():
                ...
                if position.size == 0: # 只有当仓位完全平掉时才清理
                    self.log(f'Position closed for {data_name}. Clearing states.', data=order_data)
                    ...
                    self.position_types[order_data] = None # 仓位类型也清掉
                     ...
                     self.log(f'Position size reduced for {data_name} to {position.size}. States not fully cleared.', data=order_data)
    ...
    def _calculate_trade_size(self, data_close_price, entry_price, stop_loss_price, risk_per_trade_percent, data=None):
        ...
        self.log(f"Calculated size based on risk: {size} (Raw: {size_raw:.2f})", data=data)
            ...
            # If no position, check for entry conditions
                ...
                try:
                    ...
                    is_range_confirmed = (not is_trend_up and # 必须不是上升趋势 (Must not be in an uptrend)
                                          abs(self.emas_medium[i][0] / self.emas_medium[i][-1] - 1) < 0.003 and # 中期EMA变化率小 (Small change in medium EMA)
                                          abs(self.emas_long[i][0] / self.emas_long[i][-1] - 1) < 0.003 and # 长期EMA变化率小 (Small change in long EMA)
                                          self.adxs[i].adx[0] < 20 and # ADX 指示趋势强度弱 (ADX indicates weak trend)
                ...
                if market_state == 'TREND_UP' and self.params.etf_type == 'trend':  # 使用params访问参数
                    ...
                    try:
                        ...
                        if is_breakout or is_pullback:
                            ...
                            self.log(f"Trend Entry Signal: Breakout={is_breakout}, Pullback={is_pullback}", data=d)
                            entry_signal = True
                ...
                # Range strategy entry logic
                    ...
                    try:
                        ...
                        is_range_buy = (self.lows[i][0] <= self.bbands[i].bot[0] and # 触及或下穿下轨 (Touches or penetrates lower band)
                                        self.closes[i][0] > self.bbands[i].bot[0] and # 收盘价回到下轨上方 (Closes back above lower band)
                        ...
                        if is_range_buy:
                            ...
                            self.log(f"Range Entry Signal: Low<={self.bbands[i].bot[0]:.2f}, Close>{self.bbands[i].bot[0]:.2f}, RSI<{self.params.rsi_oversold}", data=d)
                            entry_signal = True
                            # 设置入场信号为True。 (标记：可以买这只ETF了！)
