    ...
    def _getsizing(self, comminfo, cash, data, isbuy):
        ...
        if not hasattr(self.strategy, 'current_trade_risk_per_share') or \
           ...
           self.strategy.current_trade_risk_per_share <= 0:
            self.strategy.log(f"Sizer Error: Invalid or missing 'current_trade_risk_per_share' ({getattr(self.strategy, 'current_trade_risk_per_share', 'N/A')}). Cannot calculate size.", data=data)
        ...
        if position_type == 'trend':
            ...
            risk_per_trade_percent = self.p.risk_per_trade_range_p
             ...
             self.strategy.log(f"Sizer Error: Unknown position type '{position_type}' for risk calculation.", data=data)
             return 0 # 未知类型，无法确定风险比例 (Unknown type, cannot determine risk percentage)


        # 应用风险乘数 (Apply risk multiplier)
        effective_risk_percent = risk_per_trade_percent * self.strategy.current_risk_multiplier
        ...
        for d, pos in self.broker.positions.items():
            ...
            if pos.size != 0 and \
               ...
               hasattr(self.strategy, 'stop_loss_prices') and d in self.strategy.stop_loss_prices and self.strategy.stop_loss_prices[d] is not None:
                ...
                initial_buy_price = self.strategy.buy_prices[d] # 获取记录的买入价 (Get recorded buy price)
                sl_price = self.strategy.stop_loss_prices[d]     # 获取记录的止损价 (Get recorded stop-loss price)
                risk_per_share_original = initial_buy_price - sl_price # 计算原始每股风险 (Calculate original risk per share)

                if risk_per_share_original > 0:
                    total_current_risk += abs(pos.size) * risk_per_share_original # 累加风险 (Accumulate risk)
                else:
                     # 如果现有持仓的风险计算有问题，记录警告 (Log warning if existing position's risk calculation is problematic)
                    self.strategy.log(f"Sizer Warning: Original risk per share for existing position {d._name} is non-positive ({risk_per_share_original:.2f}). Buy: {initial_buy_price:.2f}, SL: {sl_price:.2f}", data=d)
        ...
        self.strategy.log(f"Sizer: Calculated size based on risk: {size} (Raw: {size_raw:.2f})", data=data)

        if size <= 0:
            self.strategy.log(f"Sizer: Calculated size is zero or negative ({size}) based on risk. Cannot place order.", data=data)
        ...
        current_price_for_calc = data.close[0] # 使用当前收盘价估算 (Use current close price for estimation)
        if current_price_for_calc <= 0:
             self.strategy.log(f"Sizer Error: Invalid price ({current_price_for_calc:.2f}) for size calculation.", data=data)
        ...
        potential_trade_value = size * current_price_for_calc # 重新计算调整后的市值 (Recalculate value after potential adjustment)
        ...
        self.strategy.log(f"Sizer: Final trade size determined: {size}", data=data)
...
class AShareETFStrategy(bt.Strategy):
    ...
    def __init__(self):
        ...
        self.current_trade_risk_per_share = None # 存储当前待交易的每股风险 (Stores risk per share for the current pending trade)
        self.current_trade_position_type = None # 存储当前待交易的仓位类型 (Stores position type for the current pending trade)
    ...
    def _calculate_trade_size(self, data_close_price, entry_price, stop_loss_price, risk_per_trade_percent, data=None):
        ...
        # **日志点**: 记录基于风险计算的原始大小
        # **Log Point**: Record the raw size calculated based on risk.
        self.log(
            f"Calculated size based on risk: {size} (Raw: {size_raw:.2f})", data=data)

        if size <= 0:
            # 检查计算出的仓位大小是否小于等于0。 (判断算出来的股数是不是小于等于0，小于等于0就说明买不了。)
            # **日志点**: 记录因风险计算导致无法下单
            # **Log Point**: Log inability to place order due to risk calculation.
            self.log(
                f"Calculated size is zero or negative ({size}) based on risk. Cannot place order.", data=data)
        ...
        # **检查点1: 最大持仓市值限制**
        # **Checkpoint 1: Max Position Value Limit**
        if potential_trade_value > max_pos_value:
            # 检查潜在交易市值是否超过单个ETF最大持仓市值限制。 (判断算出来的要买的金额是不是超过了单个ETF的上限。)
            new_size = int(max_pos_value / current_price_for_calc / 100) * 100
            # 如果超过限制，则将仓位大小调整为不超过最大持仓市值的最大100股整数倍。 (如果超过了，那就减少买入股数，只买到上限允许的金额。)
            # **日志点**: 记录因最大持仓限制调整大小
            # **Log Point**: Log size adjustment due to max position limit.
            self.log(
                f"Size reduced from {size} to {new_size} due to max position limit ({self.params.max_position_per_etf_percent*100:.0f}% of equity). Potential Value: {potential_trade_value:.2f} > Max Value: {max_pos_value:.2f}", data=data)
            # 记录日志，提示仓位大小因最大持仓限制而调整。 (记录日志，说买太多超标了，减少到多少股。)
            size = new_size
            # 更新仓位大小。 (更新买入股数。)

        potential_trade_value = size * current_price_for_calc  # 重新计算调整后的市值
        # 重新计算调整后的潜在交易市值。 (Recalculate the potential trade value after adjustment.)

        # **检查点2: 可用现金限制**
        # **Checkpoint 2: Available Cash Limit**
        if potential_trade_value > cash:
            ...
            if new_size < size:  # 只有当现金限制导致规模减小时才记录
                # **日志点**: 记录因现金限制调整大小
                # **Log Point**: Log size adjustment due to cash limit.
                self.log(
        ...
        # **最终检查**: 经过所有限制后的大小是否有效
        # **Final Check**: Is the size valid after all limits?
        if size <= 0:
            # 再次检查计算出的仓位大小是否小于等于0。 (再次检查，经过各种限制后，算出来的股数是不是小于等于0。)
            # **日志点**: 记录最终计算大小无效
            # **Log Point**: Log final calculated size is invalid.
            self.log(
                f"Final calculated size is zero or negative ({size}) after applying limits. Cannot place order.", data=data)
            # 记录日志，提示最终仓位大小为零或负数，无法下单。 (记录日志，说最终算出来还是买不了，不买了。)
            return 0
            # 如果最终仓位大小小于等于0，则返回0。 (如果最终算出来买不了，那就返回0。)

        # **日志点**: 记录最终有效大小
        # **Log Point**: Log the final valid size.
        self.log(f"Final trade size determined: {size}", data=data)
        return size
