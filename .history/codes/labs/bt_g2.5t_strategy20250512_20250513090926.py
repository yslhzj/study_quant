                ...
                # Range strategy entry logic
                    ...
                    try:
                        ...
                        # 判断是否为震荡买入信号：最低价触及或下穿布林带下轨，收盘价回到下轨之上，且RSI超卖。 (判断这只ETF是不是到底了：价格碰到或跌破布林带下轨，但当天收盘又涨回来了，并且RSI显示超卖。)
                            ...
                            if is_range_buy:
                                # ... (设置 entry_signal, potential_position_type, 计算 SL/TP 和 risk_per_share) ...
                                if stop_loss_price_calc < entry_price_calc:
                                    risk_per_share = entry_price_calc - stop_loss_price_calc
                                    if risk_per_share > 0:
                                        self.current_trade_risk_per_share = risk_per_share
                                        self.current_trade_position_type = potential_position_type
                                        self.stop_loss_prices[d] = stop_loss_price_calc
                                        self.take_profit_prices[d] = take_profit_price_calc
                                    else:
                                         entry_signal = False # 如果风险计算有问题，则取消信号
                                else:
                                     entry_signal = False # 如果止损价计算有问题，则取消信号
                        except IndexError:
                            continue # 数据不足，跳过ipping {data_name} range check: Not enough data.', data=d)
                        continue

                # 如果有入场信号并且计算出的止损价有效
                # If there is an entry signal and the calculated stop-loss price is valid
                    if entry_signal and self.current_trade_risk_per_share is not None and self.current_trade_risk_per_share > 0:
                        # **核心修复**: 删除下面这行对 _calculate_trade_size 的调用
                        # **Core Fix**: Delete the following line calling _calculate_trade_size
                        # size = self._calculate_trade_size( ... ) # <- 删除此行 / DELETE THIS LINE (Line 764 according to trace)

                        self.log(
                            f'BUY SIGNAL (Let Sizer decide size): {data_name}, Entry: ~{entry_price_calc:.2f}, SL: {stop_loss_price_calc:.2f}, TP: {take_profit_price_calc if take_profit_price_calc else "N/A"}, Type: {potential_position_type}', data=d)

                        # 调用 buy_bracket，不传递 size 参数，让 Sizer 决定
                        # Call buy_bracket without the size argument, let Sizer decide
                        bracket_orders = self.buy_bracket(
                            data=d,
                            price=entry_price_calc,
                            stopprice=stop_loss_price_calc,
                            limitprice=take_profit_price_calc
                        )
                        ...
                        if bracket_orders and bracket_orders[0]:
                            ...
                            self.log(f'Failed to create buy_bracket order for {data_name}', data=d)
