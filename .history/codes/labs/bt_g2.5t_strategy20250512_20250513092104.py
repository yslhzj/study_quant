...
# ===================================================================================
# 自定义 Sizer (Custom Sizer)
# ===================================================================================
class RiskAdjustedSizer(bt.Sizer):
    """
    根据单笔风险、总账户风险、最大单标仓位和现金进行仓位计算的Sizer。
    Calculates position size based on per-trade risk, total account risk,
    max position per symbol, and cash availability.

    需要策略在调用buy/sell前，将计算好的每股风险存储在 self.strategy.current_trade_risk_per_share 中。
    Requires the strategy to store the calculated risk per share in
    `self.strategy.current_trade_risk_per_share` before calling buy/sell.
    """
    params = (
        ('risk_per_trade_trend_p', 0.01),  # 趋势策略单笔风险 % (从策略参数获取)
        ('risk_per_trade_range_p', 0.005),  # 震荡策略单笔风险 % (从策略参数获取)
        ('max_position_p', 0.30),          # 最大单标仓位 % (从策略参数获取)
        ('max_total_risk_p', 0.06),        # 最大账户总风险 % (从策略参数获取)
    )

    def _getsizing(self, comminfo, cash, data, isbuy):
        # isbuy 参数在此 Sizer 中未使用，因为风险计算通常基于止损距离
        # The isbuy parameter is not used in this Sizer, as risk calculation
        # is typically based on stop-loss distance.

        # 检查策略是否传递了必要的风险信息
        # Check if the strategy passed the necessary risk information
        if not hasattr(self.strategy, 'current_trade_risk_per_share') or \
           self.strategy.current_trade_risk_per_share is None or \
           self.strategy.current_trade_risk_per_share <= 0:
            self.strategy.log(
                f"Sizer Error: Invalid or missing 'current_trade_risk_per_share' ({getattr(self.strategy, 'current_trade_risk_per_share', 'N/A')}). Cannot calculate size.", data=data)
            return 0  # 无法计算风险，返回0大小 (Cannot calculate risk, return size 0)

        risk_per_share = self.strategy.current_trade_risk_per_share
        # 获取策略计算的每股风险。 (Get the risk per share calculated by the strategy.)
        position_type = self.strategy.current_trade_position_type  # 获取策略决定的仓位类型
        # 获取策略决定的仓位类型（'trend' 或 'range'）。 (Get the position type determined by the strategy ('trend' or 'range').)

        # 根据仓位类型选择风险比例
        # Select risk percentage based on position type
        if position_type == 'trend':
            risk_per_trade_percent = self.p.risk_per_trade_trend_p
        elif position_type == 'range':
            risk_per_trade_percent = self.p.risk_per_trade_range_p
        else:
            self.strategy.log(
                f"Sizer Error: Unknown position type '{position_type}' for risk calculation.", data=data)
            # 未知类型，无法确定风险比例 (Unknown type, cannot determine risk percentage)
            return 0

        # 应用风险乘数 (Apply risk multiplier)
        effective_risk_percent = risk_per_trade_percent * \
            self.strategy.current_risk_multiplier
        # 计算有效的单笔交易风险比例，考虑当前的风险乘数。 (Calculate the effective per-trade risk percentage, considering the current risk multiplier.)

        # 获取账户净值 (Get account equity)
        equity = self.broker.getvalue()
        # 获取当前账户的总净值（现金+持仓市值）。 (Get the current total equity (cash + portfolio value).)

        # --- 总账户风险检查 (Total Account Risk Check) ---
        total_current_risk = 0
        # 初始化当前账户总风险为0。 (Initialize current total account risk to 0.)
        # 遍历现有持仓计算已承担风险 (Iterate through existing positions to calculate committed risk)
        for d, pos in self.broker.positions.items():
            # 检查是否有头寸，并且策略中记录了对应的买入价和止损价
            # Check for position and if buy/stop prices are recorded in the strategy
            if pos.size != 0 and \
               hasattr(self.strategy, 'buy_prices') and d in self.strategy.buy_prices and self.strategy.buy_prices[d] is not None and \
               hasattr(self.strategy, 'stop_loss_prices') and d in self.strategy.stop_loss_prices and self.strategy.stop_loss_prices[d] is not None:

                # 获取记录的买入价 (Get recorded buy price)
                initial_buy_price = self.strategy.buy_prices[d]
                # 获取记录的止损价 (Get recorded stop-loss price)
                sl_price = self.strategy.stop_loss_prices[d]
                # 计算原始每股风险 (Calculate original risk per share)
                risk_per_share_original = initial_buy_price - sl_price

                if risk_per_share_original > 0:
                    # 累加风险 (Accumulate risk)
                    total_current_risk += abs(pos.size) * \
                        risk_per_share_original
                else:
                    # 如果现有持仓的风险计算有问题，记录警告 (Log warning if existing position's risk calculation is problematic)
                    self.strategy.log(
                        f"Sizer Warning: Original risk per share for existing position {d._name} is non-positive ({risk_per_share_original:.2f}). Buy: {initial_buy_price:.2f}, SL: {sl_price:.2f}", data=d)

        # 计算潜在新交易的风险金额 (Calculate potential new trade risk amount)
        potential_new_risk = (equity * effective_risk_percent)
        # 判断加入新交易后是否超过总风险上限 (Check if adding the new trade exceeds the total risk limit)
        if (total_current_risk + potential_new_risk) / equity > self.p.max_total_risk_p:
            self.strategy.log(
                f"Sizer Skipped: Trade exceeds max total account risk limit. Current risk: {(total_current_risk/equity)*100:.2f}%, New trade risk: {effective_risk_percent*100:.2f}%, Limit: {self.p.max_total_risk_p*100:.2f}%", data=data)
            return 0  # 超过总风险上限，返回0大小 (Exceeds total risk limit, return size 0)
        # --- 总账户风险检查结束 ---

        # 计算本次交易允许的最大风险金额 (Calculate the maximum risk amount allowed for this trade)
        risk_amount = equity * effective_risk_percent
        # 计算理论上的仓位大小（股数） (Calculate the theoretical position size in shares)
        size_raw = risk_amount / risk_per_share
        # 向下取整到100的倍数 (Round down to the nearest multiple of 100)
        size = int(size_raw / 100) * 100

        # **日志点**: 记录基于风险计算的原始大小 (Log Point: Record raw size based on risk)
        self.strategy.log(
            f"Sizer: Calculated size based on risk: {size} (Raw: {size_raw:.2f})", data=data)

        if size <= 0:
            self.strategy.log(
                f"Sizer: Calculated size is zero or negative ({size}) based on risk. Cannot place order.", data=data)
            return 0  # 大小无效，返回0 (Invalid size, return 0)

        # --- 最大单标仓位限制检查 (Max Position per Symbol Limit Check) ---
        max_pos_value = equity * self.p.max_position_p
        # 计算单个ETF允许的最大持仓市值。 (Calculate the maximum allowed position value for a single ETF.)
        # 使用当前收盘价估算 (Use current close price for estimation)
        current_price_for_calc = data.close[0]
        if current_price_for_calc <= 0:
            self.strategy.log(
                f"Sizer Error: Invalid price ({current_price_for_calc:.2f}) for size calculation.", data=data)
            return 0  # 价格无效，返回0 (Invalid price, return 0)

        potential_trade_value = size * current_price_for_calc
        # 计算潜在交易的总市值 (Calculate the potential trade value)

        if potential_trade_value > max_pos_value:
            # 如果超过限制，调整大小 (If limit exceeded, adjust size)
            new_size = int(max_pos_value / current_price_for_calc / 100) * 100
            # **日志点**: 记录因最大持仓限制调整大小 (Log Point: Log size adjustment due to max position limit)
            self.strategy.log(
                f"Sizer: Size reduced from {size} to {new_size} due to max position limit ({self.p.max_position_p*100:.0f}% of equity). Potential Value: {potential_trade_value:.2f} > Max Value: {max_pos_value:.2f}", data=data)
            size = new_size  # 更新大小 (Update size)
        # --- 最大单标仓位限制检查结束 ---

        # --- 可用现金限制检查 (Available Cash Limit Check) ---
        # 重新计算调整后的市值 (Recalculate value after potential adjustment)
        potential_trade_value = size * current_price_for_calc
        if potential_trade_value > cash:
            # 如果市值超过现金 (If value exceeds cash)
            new_size = int(cash / current_price_for_calc / 100) * 100
            # 只有当现金限制导致规模减小时才记录 (Only log if cash limit causes reduction)
            if new_size < size:
                # **日志点**: 记录因现金限制调整大小 (Log Point: Log size adjustment due to cash limit)
                self.strategy.log(
                    f"Sizer: Size reduced from {size} to {new_size} due to cash limit (Available: {cash:.2f}, Required: {potential_trade_value:.2f}).", data=data)
                size = new_size  # 更新大小 (Update size)
        # --- 可用现金限制检查结束 ---

        # --- 最终大小检查 (Final Size Check) ---
        if size <= 0:
            self.strategy.log(
                f"Sizer: Final calculated size is zero or negative ({size}) after applying limits. Cannot place order.", data=data)
            return 0  # 最终大小无效，返回0 (Final size invalid, return 0)

        # **日志点**: 记录最终有效大小 (Log Point: Log the final valid size)
        self.strategy.log(
            f"Sizer: Final trade size determined: {size}", data=data)
        return size  # 返回最终计算的大小 (Return the final calculated size)
...
class AShareETFStrategy(bt.Strategy):
    ...
    def log(self, txt, dt=None, data=None):
        ...
        # return  # <- 注释掉或移除这行以启用日志记录 (Comment out or remove this line to enable logging)
    ...
    def __init__(self):
        ...
        # 用于 Sizer 的临时变量 (Temporary variables for the Sizer)
        # 存储当前待交易的每股风险 (Stores risk per share for the current pending trade)
        self.current_trade_risk_per_share = None
        # 存储当前待交易的仓位类型 (Stores position type for the current pending trade)
        self.current_trade_position_type = None
    ...
    def notify_order(self, order):
        ...
        if order.status in [order.Completed]:
            ...
            if order.isbuy():
                ...
                # **核心逻辑点**: 这里处理的是主买入订单(来自buy_bracket的第一部分)或独立的买入订单
                # **Core Logic Point**: Handling the main buy order (from buy_bracket's first part) or standalone buy orders.
                self.log(
                ...
                # **注意**: 对于 buy_bracket 创建的订单, 其止损/止盈订单是由 Backtrader/Broker 自动管理的。
                # 不需要在此处手动记录 self.stop_loss_prices 或 self.take_profit_prices。
                # 这些价格是在 next() 方法中计算并传递给 buy_bracket 的参数。
                # **Note**: For orders created by `buy_bracket`, the associated stop-loss/take-profit orders
                # are managed automatically by Backtrader/Broker. There's no need to manually record
                # `self.stop_loss_prices` or `self.take_profit_prices` here. Those prices were calculated
                # in the `next()` method and passed as arguments to `buy_bracket`.

            elif order.issell():
                # 检查已完成的订单是否为卖出订单。 (判断成交的订单是不是卖出单。)
                # **核心逻辑点**: 这个卖出可能是止损、止盈或者独立的卖出指令触发的
                # **Core Logic Point**: This sell could be triggered by a stop-loss, take-profit, or a standalone sell command.
                self.log(
                ...
                # 清理与该持仓相关的信息，因为头寸已被关闭或减少
                # Clear information related to this position as it has been closed or reduced.
                # **注意**: 这里的清理假设卖出意味着平仓。如果策略允许减仓，需要更复杂的逻辑。
                # **Note**: This clearing assumes the sell means closing the position. More complex logic is needed if partial closes are allowed.
                position = self.getposition(order_data)  # 获取当前仓位
                if position.size == 0:  # 只有当仓位完全平掉时才清理
                    self.log(
                        f'Position closed for {data_name}. Clearing states.', data=order_data)
                    self.buy_prices[order_data] = None
                    self.buy_comms[order_data] = None
                    self.stop_loss_prices[order_data] = None  # 理论上在平仓时这些也该清掉
                    self.take_profit_prices[order_data] = None  # 理论上在平仓时这些也该清掉
                    self.position_types[order_data] = None  # 仓位类型也清掉
                else:
                    self.log(
                        f'Position size reduced for {data_name} to {position.size}. States not fully cleared.', data=order_data)
            ...
            # **重要**: 如果是括号订单的某个子订单失败了，需要考虑是否取消其他关联订单，但这通常由Broker处理。
            # **Important**: If a child order of a bracket fails, consider if associated orders should be cancelled (usually handled by the Broker).

        # 不论订单状态如何，只要不是 PENDING 或 Accepted/Submitted (我们已提前return) 状态，就清除订单引用
        # Regardless of the order status, clear the order reference if it's no longer pending or accepted/submitted (we returned earlier).
    ...
    # 新增：用于计算交易大小的方法，逻辑类似 Sizer
    # New: Method to calculate trade size, logic similar to Sizer
    def _calculate_trade_size(self, current_price_for_calc, entry_price_calc, stop_loss_price_calc, potential_position_type, data):
        # 计算本次潜在交易的每股风险
        # Calculate risk per share for this potential trade
        risk_per_share = entry_price_calc - stop_loss_price_calc
        # 检查每股风险是否有效（必须大于0）
        # Check if risk per share is valid (must be > 0)
        if risk_per_share <= 0:
            self.log(
                f"Size Calc Error: Risk per share is non-positive ({risk_per_share:.2f}). Entry: {entry_price_calc:.2f}, SL: {stop_loss_price_calc:.2f}", data=data)
            # 如果风险无效，则无法计算大小，返回0 (If risk is invalid, cannot calculate size, return 0)
            return 0

        # 根据潜在仓位类型确定单笔风险百分比
        # Determine per-trade risk percentage based on potential position type
        if potential_position_type == 'trend':
            # 使用策略参数 (Use strategy params)
            risk_per_trade_p = self.params.max_risk_per_trade_trend
        elif potential_position_type == 'range':
            # 使用策略参数 (Use strategy params)
            risk_per_trade_p = self.params.max_risk_per_trade_range
        else:
            self.log(
                f"Size Calc Error: Unknown position type '{potential_position_type}'", data=data)
            return 0  # 未知类型，返回0 (Unknown type, return 0)

        # 应用当前风险乘数（可能因回撤而被调整）
        # Apply the current risk multiplier (may be adjusted due to drawdown)
        effective_risk_percent = risk_per_trade_p * self.current_risk_multiplier

        # 获取账户净值和可用现金
        # Get account equity and available cash
        equity = self.broker.getvalue()
        cash = self.broker.getcash()

        # --- 总账户风险检查 ---
        # --- Total Account Risk Check ---
        total_current_risk = 0
        # 初始化当前已承担的总风险 (Initialize current committed total risk)
        # 遍历现有持仓计算风险
        # Iterate through existing positions to calculate risk
        for d_pos, pos in self.broker.positions.items():
            # 检查是否有持仓，并且有记录的买入价和止损价
            # Check for position and recorded buy/stop prices
            if pos.size != 0 and d_pos in self.buy_prices and self.buy_prices[d_pos] is not None and d_pos in self.stop_loss_prices and self.stop_loss_prices[d_pos] is not None:

                # 获取原始买入价 (Get original buy price)
                initial_buy_price = self.buy_prices[d_pos]
                # 获取原始止损价 (Get original stop-loss price)
                sl_price = self.stop_loss_prices[d_pos]
                # 计算原始每股风险 (Calculate original risk per share)
                risk_per_share_original = initial_buy_price - sl_price
                ...
                if risk_per_share_original > 0:
                    ...
                    # 如果现有持仓风险计算异常，记录警告
                    # Log warning if existing position's risk calculation is problematic
                    self.log(
                        f"Size Calc Warning: Original risk for existing pos {d_pos._name} is non-positive ({risk_per_share_original:.2f}). Buy: {initial_buy_price:.2f}, SL: {sl_price:.2f}", data=d_pos)

        # 计算新交易将带来的潜在风险
        # Calculate potential risk from the new trade
        potential_new_risk = equity * effective_risk_percent
        # 获取策略参数中的总风险上限
        # Get the total risk limit from strategy parameters
        max_total_risk_allowed_p = self.params.max_total_account_risk_percent

        # 检查加入新交易后是否会超过总风险上限
        # Check if adding the new trade exceeds the total risk limit
        if (total_current_risk + potential_new_risk) / equity > max_total_risk_allowed_p:
            self.log(
                f"Size Calc Skipped: Trade exceeds max total account risk ({max_total_risk_allowed_p*100:.2f}%). Current: {(total_current_risk/equity)*100:.2f}%, New: {effective_risk_percent*100:.2f}%", data=data)
            return 0  # 超过总风险上限，返回0 (Exceeds total risk limit, return 0)

        # --- 基于单笔风险计算基础大小 ---
        # --- Calculate base size based on per-trade risk ---
        # 本次交易允许的最大风险金额 (Max risk amount allowed for this trade)
        risk_amount = equity * effective_risk_percent
        # 理论股数 (Theoretical number of shares)
        size_raw = risk_amount / risk_per_share
        # 向下取整到100的倍数 (Round down to the nearest multiple of 100)
        size = int(size_raw / 100) * 100
        ...
        # 记录日志 (Log info)
        self.log(
            f"Size Calc: Based on risk: {size} (Raw: {size_raw:.2f})", data=data)

        # 如果基于风险计算的大小无效，返回0
        # If size based on risk is invalid, return 0
        if size <= 0:
            self.log(
                f"Size Calc: Size is zero or negative ({size}) based on risk.", data=data)
            return 0

        # --- 最大单标仓位限制检查 ---
        # --- Max Position per Symbol Limit Check ---
        # 单个标的最大允许市值 (Max allowed value per symbol)
        max_pos_value = equity * self.params.max_position_per_etf_percent
        # 检查用于计算的价格是否有效
        # Check if the price used for calculation is valid
        if current_price_for_calc <= 0:
            self.log(
                f"Size Calc Error: Invalid price ({current_price_for_calc:.2f})", data=data)
            return 0  # 价格无效，返回0 (Invalid price, return 0)

        # 计算潜在交易的总市值 (Calculate potential trade value)
        potential_trade_value = size * current_price_for_calc

        # 如果潜在市值超过单标限制，则缩减大小
        # If potential value exceeds per-symbol limit, reduce size
        if potential_trade_value > max_pos_value:
            # 重新计算大小 (Recalculate size)
            new_size = int(max_pos_value / current_price_for_calc / 100) * 100
            self.log(
                f"Size Calc: Reduced from {size} to {new_size} due to max position limit ({self.params.max_position_per_etf_percent*100:.0f}%). Potential: {potential_trade_value:.2f} > Max: {max_pos_value:.2f}", data=data)
            size = new_size  # 更新大小 (Update size)

        # --- 可用现金限制检查 ---
        # --- Available Cash Limit Check ---
        # 重新计算调整后的市值 (Recalculate value after potential adjustment)
        potential_trade_value = size * current_price_for_calc
        # 如果所需资金超过可用现金，则缩减大小
        # If required funds exceed available cash, reduce size
        if potential_trade_value > cash:
            # 基于现金重新计算大小 (Recalculate size based on cash)
            new_size = int(cash / current_price_for_calc / 100) * 100
            # 仅当现金限制导致规模真实减小时才记录日志
            # Only log if the cash limit actually causes a size reduction
            if new_size < size:
                self.log(
                    f"Size Calc: Reduced from {size} to {new_size} due to cash limit (Available: {cash:.2f}, Required: {potential_trade_value:.2f}).", data=data)
                size = new_size  # 更新大小 (Update size)

        # --- 最终大小检查 ---
        # --- Final Size Check ---
        # 如果经过所有限制调整后，最终大小无效，返回0
        # If the final size is invalid after all adjustments, return 0
        if size <= 0:
            self.log(
                f"Size Calc: Final size is zero or negative ({size}).", data=data)
            return 0

        # 记录最终计算的大小 (Log the final calculated size)
        self.log(f"Size Calc: Final trade size determined: {size}", data=data)
        return size  # 返回最终计算的大小 (Return the final calculated size)
    ...
    def next(self):
        ...
        for i, d in enumerate(self.datas):
            ...
            # If no position, check for entry conditions
                ...
                try:
                    # **核心判断: 市场状态识别**
                    # **Core Logic: Market State Identification**
                    # 识别当前市场是上升趋势还是震荡市
                    # Identify if the current market is in an uptrend or ranging.

                    # **上升趋势条件**: 收盘价 > 中期EMA > 长期EMA, 且两条EMA线均向上
                    # **Uptrend Condition**: Close > Medium EMA > Long EMA, and both EMAs are rising.
                    ...
                    # **震荡确认条件**: 非上升趋势 + 均线变化平缓 + ADX低 + 布林带窄
                    # **Range Confirmation**: Not uptrend + EMAs moving sideways + Low ADX + Narrow Bollinger Bands.
                    is_range_confirmed = (not is_trend_up and  # 必须不是上升趋势 (Must not be in an uptrend)
                                          # 中期EMA变化率小 (Small change in medium EMA)
                                          abs(self.emas_medium[i][0] / self.emas_medium[i][-1] - 1) < 0.003 and
                                          # 长期EMA变化率小 (Small change in long EMA)
                                          abs(self.emas_long[i][0] / self.emas_long[i][-1] - 1) < 0.003 and
                                          # ADX 指示趋势强度弱 (ADX indicates weak trend)
                                          self.adxs[i].adx[0] < 20 and
                                          (self.bbands[i].top[0] - self.bbands[i].bot[0]) / self.closes[i][0] < 0.07)  # 布林带宽度占价格比例小 (Bollinger Band width is small relative to price)
                    ...
                    # else: 默认是 'UNCERTAIN_DO_NOT_TRADE' (Default is 'UNCERTAIN_DO_NOT_TRADE')
                ...
                if market_state == 'TREND_UP' and self.params.etf_type == 'trend':  # 使用params访问参数
                    ...
                    try:
                        # **核心判断: 趋势入场信号**
                        # **Core Logic: Trend Entry Signal**

                        # **突破信号**: 收盘价 > 前N日最高价 + 成交量 > 前M日均量 * 比例
                        # **Breakout Signal**: Close > Highest High of lookback period + Volume > Avg Volume * Ratio
                        ...
                        # **回调企稳信号**: 最低价接近任一EMA均线 + 当日收阳线 (或收盘价高于某个EMA)
                        # **Pullback Stabilization Signal**: Low near either EMA + Closed positive for the day (or Close above an EMA).
                        # 这里判断最低价接近均线的阈值为1%
                        # Using a 1% threshold for low price proximity to EMAs.
                        is_pullback = (min(abs(self.lows[i][0] / self.emas_medium[i][0] - 1), abs(self.lows[i][0] / self.emas_long[i][0] - 1)) < 0.01 and
                                       self.closes[i][0] > self.opens[i][0])  # 收阳线 (Closed positive)
                        # 判断是否为回调企稳信号：当日最低价接近均线，且当日收阳线。 (判断这只ETF是不是回调站稳了：价格跌到均线附近，但当天又涨回来了。)

                        if is_breakout or is_pullback:
                            # 如果出现突破信号或回调企稳信号。 (如果突破了或者回调站稳了。)
                            self.log(
                                f"Trend Entry Signal: Breakout={is_breakout}, Pullback={is_pullback}", data=d)
                            entry_signal = True
                ...
                # Range strategy entry logic
                    ...
                    try:
                        # **核心判断: 震荡买入信号**
                        # **Core Logic: Range Buy Signal**

                        # **震荡买入条件**: 最低价 <= 布林下轨 + 收盘价 > 布林下轨 + RSI < 超卖线
                        # **Range Buy Condition**: Low <= BB Lower + Close > BB Lower + RSI < Oversold threshold
                        is_range_buy = (self.lows[i][0] <= self.bbands[i].bot[0] and  # 触及或下穿下轨 (Touches or penetrates lower band)
                                        # 收盘价回到下轨上方 (Closes back above lower band)
                                        self.closes[i][0] > self.bbands[i].bot[0] and
                                        self.rsis[i][0] < self.params.rsi_oversold)  # RSI 指标超卖 (RSI indicates oversold)
                        # 判断是否为震荡买入信号：最低价触及或下穿布林带下轨，收盘价回到下轨之上，且RSI超卖。 (判断这只ETF是不是到底了：价格碰到或跌破布林带下轨，但当天收盘又涨回来了，并且RSI显示超卖。)

                        if is_range_buy:
                            # 如果出现震荡买入信号。 (如果满足上面的条件。)
                            self.log(
                                f"Range Entry Signal: Low<={self.bbands[i].bot[0]:.2f}, Close>{self.bbands[i].bot[0]:.2f}, RSI<{self.params.rsi_oversold}", data=d)
                            entry_signal = True
                            # 设置入场信号为True。 (标记：可以买这只ETF了！)
                ...
                if entry_signal and stop_loss_price_calc is not None and entry_price_calc > stop_loss_price_calc:
                    ...
                    # **核心修改点**: 在调用大小计算前，先确认每股风险有效
                    # **Core Modification**: Before calling size calculation, ensure risk per share is valid
                    risk_per_share = entry_price_calc - stop_loss_price_calc
                    if risk_per_share <= 0:
                        # 如果每股风险无效 (例如止损价高于或等于入场价)
                        # If risk per share is invalid (e.g., stop-loss is at or above entry)
                        self.log(f"Signal {data_name} skipped due to invalid risk per share: Entry {entry_price_calc:.2f}, SL {stop_loss_price_calc:.2f}", data=d)
                        continue # 跳过此信号，处理下一个ETF或下一个时间点 (Skip this signal, process next ETF or next time step)

                    # **核心修改点**: 调用内部实现的 _calculate_trade_size 方法，并传入正确的参数
                    # **Core Modification**: Call the internally implemented _calculate_trade_size method with correct parameters
                    size = self._calculate_trade_size(
                        current_price_for_calc=self.closes[i][0],  # 使用当前收盘价进行市值和现金检查 (Use current close for value and cash checks)
                        entry_price_calc=entry_price_calc,        # 计划的入场价格 (Planned entry price)
                        stop_loss_price_calc=stop_loss_price_calc,  # 计算出的止损价格 (Calculated stop-loss price)
                        potential_position_type=potential_position_type, # 潜在的仓位类型 ('trend' or 'range') (Potential position type)
                        data=d)
                    ...
                    if size > 0:
                        ...
                        bracket_orders = self.buy_bracket(
                            data=d,
                            size=size, # **重要**: 此处传入计算好的size (Important: Pass the calculated size here)
                        ...
                        if bracket_orders and bracket_orders[0]:
                            ...
                            # **重要**: 成功下单后，需要记录实际的止损价和买入价，供后续总风险计算使用。
                            # **Important**: After successful order placement, record actual SL and buy price for future total risk calc.
                            # 注意: buy_bracket 的主订单成交后，notify_order会记录buy_price。但止损价是预设的。
                            # Note: notify_order records buy_price after main order execution. SL is preset.
                            self.stop_loss_prices[d] = stop_loss_price_calc
                            # 理论上，止盈价也应记录，但当前逻辑是关闭后清理。
                            # Theoretically, take_profit should also be recorded, but current logic clears it on close.
                            if take_profit_price_calc:
