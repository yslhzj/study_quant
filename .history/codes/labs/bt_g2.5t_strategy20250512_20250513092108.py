            ...
            # If no position, check for entry conditions
                ...
                if entry_signal and stop_loss_price_calc is not None and entry_price_calc > stop_loss_price_calc:
                    ...
                    if risk_per_share <= 0:
                        ...
                        self.log(
                            f"Signal {data_name} skipped due to invalid risk per share: Entry {entry_price_calc:.2f}, SL {stop_loss_price_calc:.2f}", data=d)
                        # 跳过此信号，处理下一个ETF或下一个时间点 (Skip this signal, process next ETF or next time step)
                        continue
                    ...
                    size = self._calculate_trade_size(
                        # 使用当前收盘价进行市值和现金检查 (Use current close for value and cash checks)
                        current_price_for_calc=self.closes[i][0],
                        # 计划的入场价格 (Planned entry price)
                        entry_price_calc=entry_price_calc,
                        # 计算出的止损价格 (Calculated stop-loss price)
                        stop_loss_price_calc=stop_loss_price_calc,
                        # 潜在的仓位类型 ('trend' or 'range') (Potential position type)
                        potential_position_type=potential_position_type,
                        data=d)
                    ...
                    if size > 0:
                        ...
                        bracket_orders = self.buy_bracket(
                            data=d,
                            # **重要**: 此处传入计算好的size (Important: Pass the calculated size here)
                            size=size,
