...
class AShareETFStrategy(bt.Strategy):
    ...
    def next(self):
        ...
        for i, d_obj in enumerate(self.datas):
            ...
            if entry_signal and entry_price_calc is not None and stop_loss_price_calc is not None and risk_per_share is not None and amount_to_risk is not None:
                # 检查所有必要信息是否都已计算完毕
                # (Check if all necessary info has been calculated)
                ...
                elif potential_position_type == 'trend':
                    self.log(f"警告 for {d_name}: 趋势交易止盈价无效或未设置 ({take_profit_price_calc})。括号单将无止盈限价单。", data=d_obj)
                    # (Warning for {d_name}: Trend trade TP price invalid or not set ({take_profit_price_calc}). Bracket will have no limit sell for TP.)

                # 调用 buy_bracket，Sizer 会自动被调用来计算 size
                # (Call buy_bracket, Sizer will be called automatically to calculate size)
                self.log(f"发出买入括号单信号 for {d_name}", data=d_obj)
                ...
                if bracket_orders_list and bracket_orders_list[0]:
                    ...
                    self.log(f"成功为 {d_name} 创建 buy_bracket 请求。主订单 ref: {bracket_orders_list[0].ref}", data=d_obj)
                    # (Successfully created buy_bracket request for {d_name}. Main order ref: {bracket_orders_list[0].ref})
                else:
