...
if __name__ == '__main__':
    ...
    # 创建Cerebro引擎实例。
    # `stdstats=False` 表示不自动添加标准统计分析器和绘图观察器。
    # `optreturn=False` 表示在优化模式下，`cerebro.run()` 返回一个包含所有策略运行结果的列表的列表，而不是只返回优化后的参数。
    # (创建 `backtrader` 的大脑 `cerebro`。 `stdstats=False` 的意思是不会自动加一些标准的统计分析和绘图观察器；`optreturn=False` 的意思是优化的时候，`run()` 会把每次跑的结果都返回来，而不是只给最好的那个。)
