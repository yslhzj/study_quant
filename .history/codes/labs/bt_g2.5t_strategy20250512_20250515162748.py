...
class AShareETFSizer(bt.Sizer):
    ...
    # (创建一个专门给A股ETF计算每次买卖多少份的工具，这个工具是基于 `backtrader` 里面一个叫 `Sizer` 的基础工具改造的。)
    params = (
        # 定义该Sizer的参数。
        # (这里可以设置一些这个计算工具在工作时会用到的选项，比如下面这个就是其中一个选项。)
        ('max_position_per_etf_percent', 0.30),
        # 定义参数 `max_position_per_etf_percent`，默认值为0.30 (30%)，表示单个ETF持仓市值占总账户价值的最大比例。
        # (设置一个名叫 'max_position_per_etf_percent' 的选项，默认值是0.30，意思是单个ETF最多能买到占你总资产30%的市值，这个限制由这个Sizer自己来控制。)
    )

    def _getsizing(self, comminfo, cash, data, isbuy):
        # 定义 `_getsizing` 方法，此方法由Backtrader在需要确定头寸大小时调用。
        # (定义一个名叫 `_getsizing` 的函数，`backtrader` 每次要决定买多少股票的时候，就会来调用这个函数来计算。)

        # 代码 1-15行: 初始化检查与准备工作
        # (这部分代码是做一些初步的检查和准备，看看能不能继续往下算。)
        if not isbuy:
            # 检查当前操作是否为买入操作。
            # (判断一下当前是不是要买入。)
            return 0
            # 如果不是买入操作，则返回0，表示不进行任何头寸调整（Sizer只处理买入逻辑）。
            # (如果不是买入，那就返回0，意思是不买，这个工具只管买的时候算数量。)

        position = self.broker.getposition(data)
        # 获取当前交易品种（由`data`对象表示）的持仓情况。
        # (查一下现在手上有没有这只ETF，有多少。)
        if position.size != 0:
            # 检查是否已持有该品种的仓位。
            # (如果已经持有这只ETF了，即持仓数量不为0。)
            return 0
            # 如果已持有仓位，则返回0，表示不重复开仓。
            # (那就返回0，意思是不再买了，避免重复开仓。)

        d_name = data._name
        # 获取当前数据对象（例如某ETF）的名称。
        # (拿到这只ETF的名字，比如“华夏沪深300ETF”。)
        strategy = self.strategy
        # 获取关联的策略实例，Sizer需要从策略中获取一些决策信息。
        # (拿到咱们正在用的那个交易策略的实例，因为有些信息要从策略那里拿。)

        # 代码 16-47行: 从策略获取并验证预计算的风险信息
        # (这部分代码是从交易策略那里拿到预先算好的风险数据，并检查这些数据是不是有效。)
        if not hasattr(strategy, 'pending_trade_info') or d_name not in strategy.pending_trade_info:
            # 检查策略实例中是否存在名为 `pending_trade_info` 的属性（通常是一个字典），
            # 并且检查该字典中是否包含当前ETF (`d_name`) 的待处理交易信息。
            # (看看策略那边有没有准备好一个叫 'pending_trade_info' 的本子（字典），以及本子里有没有记录当前这只ETF的交易计划。)
            strategy.log(
                # 使用策略的log方法记录日志。
                # (用策略的日志功能记一笔。)
                f"Sizer: 无法在 strategy.pending_trade_info 中找到 {d_name} 的待处理交易信息。跳过。", data=data)
                # 记录日志，说明在策略的 `pending_trade_info` 中找不到当前ETF的交易信息，因此跳过此次头寸计算。
                # (如果找不到，就记个日志说：“Sizer：在策略的pending_trade_info里找不到 {ETF名称} 的交易计划，这次不买了。”)
            return 0
            # 返回0，表示不进行交易。
            # (返回0，不买了。)
        ...
        # (从策略的 'pending_trade_info' 本子里拿出这只ETF的交易计划详情。)

        entry_price = trade_info.get('entry_price')
        # 从获取的交易信息中提取 'entry_price'（预估入场价格）。
        # (从交易计划里拿到预估的买入价格。)
        risk_per_share = trade_info.get('risk_per_share')
        # 从获取的交易信息中提取 'risk_per_share'（每股预期风险）。
        # (从交易计划里拿到买一股可能亏多少钱的预估值。)
        amount_to_risk = trade_info.get('amount_to_risk')
        # 从获取的交易信息中提取 'amount_to_risk'（本次交易愿意承担的总风险金额）。
        # (从交易计划里拿到这次交易总共愿意亏多少钱的上限。)

        if entry_price is None or risk_per_share is None or amount_to_risk is None:
            # 检查从策略获取的关键交易信息（入场价、每股风险、总风险金额）是否有缺失。
            # (看看是不是缺了买入价、每股风险、或者总风险金额这些关键信息。)
            strategy.log(
                # 使用策略的log方法记录日志。
                # (用策略的日志功能记一笔。)
                f"Sizer: 从策略获取的 {d_name} 交易信息不完整。跳过。信息: {trade_info}", data=data)
                # 记录日志，说明从策略获取的交易信息不完整。
                # (如果信息不全，就记个日志说：“Sizer：从策略拿到的 {ETF名称} 交易信息不全，不买了。具体信息是：{交易计划详情}”。)
            del strategy.pending_trade_info[d_name]
            # 从策略的 `pending_trade_info` 中删除当前ETF的无效交易信息，避免后续混淆。
            # (把策略那边对应的这条不完整的交易计划也删掉，免得以后出问题。)
            return 0
            # 返回0，表示不进行交易。
            # (返回0，不买了。)

        if risk_per_share <= 1e-9:
            # 检查策略计算的每股风险是否过小（接近或等于0）。1e-9用于避免浮点数精度问题。
            # (如果策略算出来每股风险非常非常小，几乎是0。)
            strategy.log(
                # 使用策略的log方法记录日志。
                # (用策略的日志功能记一笔。)
                f"Sizer: 策略计算的 {d_name} 每股风险({risk_per_share:.2f})过小。跳过。", data=data)
                # 记录日志，说明每股风险过小，不进行交易。
                # (就记个日志说：“Sizer：策略算出来的 {ETF名称} 每股风险({具体风险数值})太小了，不买了。”)
            del strategy.pending_trade_info[d_name]
            # 从策略的 `pending_trade_info` 中删除当前ETF的交易信息。
            # (把策略那边对应的这条交易计划删掉。)
            return 0
            # 返回0，表示不进行交易。
            # (返回0，不买了。)
        if amount_to_risk <= 1e-9:
            ...
            # (如果策略算出来这次交易能承担的总风险金额非常非常小，几乎是0。)
            strategy.log(
                # 使用策略的log方法记录日志。
                # (用策略的日志功能记一笔。)
                f"Sizer: 策略计算的 {d_name} 最大风险金额({amount_to_risk:.2f})过小。跳过。", data=data)
                # 记录日志，说明最大风险金额过小，不进行交易。
                # (就记个日志说：“Sizer：策略算出来的 {ETF名称} 这次交易最多能亏的钱({具体金额})太少了，不买了。”)
            del strategy.pending_trade_info[d_name]
            # 从策略的 `pending_trade_info` 中删除当前ETF的交易信息。
            # (把策略那边对应的这条交易计划删掉。)
            return 0
        ...
        # 代码 48-59行: 基于策略风险信息计算核心头寸
        # (这部分代码是根据策略提供的风险信息，计算出初步的购买数量。)
        size_raw = amount_to_risk / risk_per_share
        # 根据总风险金额和每股风险计算原始的股票数量（可能带有小数）。
        # (用“这次交易最多能亏多少钱”除以“买一股可能亏多少钱”，得到一个初步的购买数量，这个数可能是小数。)
        size = int(size_raw / 100) * 100
        # 将原始数量向下取整到最近的100的倍数，符合A股ETF交易单位（通常为100股的整数倍）。
        # (因为A股买卖ETF一般是100股起，并且是100股的整数倍，所以把上面算出来的数量向下取整到100的倍数，比如算出来是230股，就变成200股。)
        ...
        if size <= 0:
            ...
            strategy.log(
                ...
                # 记录日志，说明基于风险计算的头寸为0或负数，无法交易。
                # (就记个日志说：“Sizer：根据风险算出来 {ETF名称} 要买的数量是 {计算出的数量}。每股风险是 {每股风险值}, 最多能亏 {总风险金额}。”，这种情况说明买不了。)
            del strategy.pending_trade_info[d_name]
            # 从策略的 `pending_trade_info` 中删除当前ETF的交易信息。
            # (把策略那边对应的这条交易计划删掉。)
            return 0
        ...
        # 代码 60-83行: 应用Sizer自身的全局限制（单个ETF最大持仓比例）
        # (这部分代码是应用Sizer自己设定的一个全局规则，就是单个ETF不能买太多，不能超过总资产的一定比例。)
        equity = self.broker.getvalue()
        # 获取当前账户总净值。
        # (看看现在整个账户里总共有多少钱（包括现金和股票市值）。)
        max_pos_value_for_etf = equity * self.p.max_position_per_etf_percent
        # 根据账户总净值和Sizer参数中设定的单个ETF最大持仓百分比，计算单个ETF允许的最大持仓市值。
        # (用总资产乘以之前设置的 'max_position_per_etf_percent'（比如30%），算出这只ETF最多能买多少钱的。)
        price_for_value_calc = entry_price
        # 使用策略提供的预估入场价作为计算持仓市值的价格。
        # (用策略计划的那个买入参考价来计算接下来要买的ETF的市值。)

        if price_for_value_calc <= 1e-9:
            # 检查策略提供的参考入场价是否有效（大于一个极小值）。
            # (如果策略给的买入价太小了，几乎是0，那这个价格就有问题。)
            strategy.log(
                # 使用策略的log方法记录日志。
                # (用策略的日志功能记一笔。)
                f"Sizer: 策略提供的 {d_name} 参考入场价 ({price_for_value_calc:.2f}) 无效。", data=data)
                # 记录日志，说明策略提供的入场价无效。
                # (就记个日志说：“Sizer：策略给的 {ETF名称} 参考买入价 ({具体价格}) 不对劲。”)
            del strategy.pending_trade_info[d_name]
            # 从策略的 `pending_trade_info` 中删除当前ETF的交易信息。
            # (把策略那边对应的这条交易计划删掉。)
            return 0
        ...
        if size > size_limited_by_max_etf_pos:
            ...
            # (如果前面按风险算出来的购买数量，比这里按“单个ETF最多买多少”算出来的数量还要多。)
            strategy.log(
                # 使用策略的log方法记录日志。
                # (用策略的日志功能记一笔。)
                f"Sizer: {d_name} 头寸从 {size} 减少到 {size_limited_by_max_etf_pos} (受限于 max_position_per_etf_percent)。", data=data)
                # 记录日志，说明头寸因达到单个ETF最大持仓比例而被调整。
                # (就记个日志说：“Sizer：{ETF名称} 原本想买 {原数量} 股，但因为单个ETF持仓上限的规定，现在只能买 {调整后数量} 股了。”)
        ...
        if size <= 0:
            ...
            # (如果调整完发现买不了了，数量变成0或者负数了。)
            strategy.log(f"Sizer: {d_name} 头寸在最大ETF仓位限制后为 {size}。", data=data)
            # 记录日志，说明调整后的头寸为0或负数。
            # (就记个日志说：“Sizer：{ETF名称} 在单个ETF持仓上限限制之后，能买的数量是 {调整后数量}。”，如果这个数是0，就说明买不了了。)
            del strategy.pending_trade_info[d_name]
            # 从策略的 `pending_trade_info` 中删除当前ETF的交易信息。
            # (把策略那边对应的这条交易计划删掉。)
            return 0
        ...
        # 代码 84-99行: 检查现金是否足够
        # (这部分代码是检查账户里的现金够不够买这么多ETF。)
        potential_trade_total_cost = size * price_for_value_calc
        # 根据当前确定的头寸数量和预估入场价，计算潜在交易的总成本（不含佣金等）。
        # (算一下按现在确定的购买数量和价格，大概要花多少钱。)
        if potential_trade_total_cost > cash:
            # 如果潜在交易的总成本超过了当前账户的可用现金。
            # (如果要花的钱比现在账户里能用的现金还多。)
            size_limited_by_cash = int(cash / price_for_value_calc / 100) * 100
            # 根据可用现金和预估入场价，计算能买的最大股数，并向下取整到100的倍数。
            # (那就用能用的现金，看看最多能买多少股，也向下凑个100股的整数倍。)
            if size_limited_by_cash < size:
                # 如果根据现金限制计算出的可购买股数小于当前已确定的头寸数量。
                ...
                strategy.log(
                    ...
                    # 记录日志，说明头寸因现金不足而被调整。
                    # (就记个日志说：“Sizer：{ETF名称} 原本想买 {原数量} 股，但因为钱不够（现金 {可用现金}，预估要花 {预估花费}），现在只能买 {现金限制后数量} 股了。”)
                size = size_limited_by_cash
                # 更新头寸数量为现金允许的最大数量。
                # (那就按现金能买的最大数量来买。)

        # 代码 100-109行: 最终头寸确认与返回
        # (这部分代码是做最后的检查，并返回最终计算好的购买数量。)
        if size <= 0:
            # 如果最终计算出的头寸数量小于或等于0（可能因为现金不足或其他限制）。
            # (如果经过所有限制调整后，最后算下来买不了了，数量是0或者负数。)
            strategy.log(f"Sizer: {d_name} 最终计算头寸为 {size}。无法下单。", data=data)
            # 记录日志，说明最终计算的头寸为0或负数，无法下单。
            # (就记个日志说：“Sizer：{ETF名称} 最后算出来能买的数量是 {最终数量}。买不了了。”)
            del strategy.pending_trade_info[d_name]
            # 从策略的 `pending_trade_info` 中删除当前ETF的交易信息。
            # (把策略那边对应的这条交易计划删掉。)
            return 0
        ...
        # 记录日志，Sizer成功为当前ETF计算出最终的头寸数量。
        # (记个日志，告诉大家：“Sizer给 {ETF名称} 算好了要买的数量是：{最终数量} 股 (这是根据策略给的风险信息算出来的)”。)

        # Sizer执行成功后，相关的pending_trade_info通常由策略在notify_order中根据订单状态进行清理。
        # (这个Sizer成功算出数量后，策略会在订单状态通知（notify_order）那里把 'pending_trade_info' 里的对应计划清理掉。)
        return size
        # 返回最终计算得到的头寸数量。
