...
class AShareETFStrategy(bt.Strategy):
    ...
    def next(self):
        ...
        for i, d_obj in enumerate(self.datas):
            ...
            elif market_state == 'RANGE_CONFIRMED' aetf_type == 'range':
...
def analyze_optimization_results(results):
    ...
    for strat_list in results:
        ...
        try:
            ...
            # 将包含策略实例、参数字典、夏普率、总收益率和最大回撤的字典添加到 `processed_results` 列表中。
            # (把这次回测的策略实例、参数、夏普率、收益率、最大回撤都打包成一个字典，然后加到 `processed_results` 列表里存起来。)
            successful_runs += 1
            # 成功提取结果的运行次数加1。
    ...
    for result_data in processed_results:
        ...
        if score > best_score:
            # 如果当前计算的得分 `score` 高于已知的最佳得分 `best_score`。
            # (如果这个分数比我们之前记录的“最好分数” `best_score` 还要高。)
            best_score = score
            # 更新最佳得分为当前得分。
            # (那就把这个分数更新成新的“最好分数”。)
            best_result_data = result_data
            # 更新最佳结果数据为当前结果数据。
            # (同时把当前这个结果 `result_data` 记为“最好的结果” `best_result_data`。)
    ...
    # (把最好的那个结果 `best_result_data` 和所有带分数的结果列表 `scored_results` 都返回出去。)
        ...
        if best_result:
            ...
            for res_data in all_scored_results[:min(20, len(all_scored_results))]:
                ...
                print(row_format_str.format(
                    ...
                    drawdown_val=res_data.get('drawdown', 0.0) * 100, # 转换为百分比显示
