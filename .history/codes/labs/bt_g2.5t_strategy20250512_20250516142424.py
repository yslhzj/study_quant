...
好的，我已经理解了您的需求。您希望在参数优化的最终输出中，直接看到最优参数组合在回测期间**实际执行的交易里，趋势交易和区间交易各自的统计情况**，例如各自的交易笔数。这样可以在一次优化后就清晰了解该最优参数组合的交易行为倾向。

为此，我们需要做以下几处修改：

1.  **在 `AShareETFStrategy` 策略类中**：
    *   `__init__`：初始化一个字典 `self.trade_details_by_type`，用来详细记录趋势（'trend'）、区间（'range'）以及未知类型（'unknown'）交易的盈亏（pnl）、次数（count）、盈利次数（wins）、亏损次数（losses）等。同时，初始化 `self.final_trade_stats_by_type` 用于在策略结束时存储最终统计。
    *   `notify_trade`：当一笔交易关闭时，根据 `self.position_types` 中记录的这笔交易的类型（是在趋势判断下开仓还是区间判断下开仓），更新 `self.trade_details_by_type` 中对应类型的统计数据（累加PnL、增加计数、记录盈亏次数等）。
    *   `stop`：在策略回测结束时，将 `self.trade_details_by_type` 的最终状态复制到 `self.final_trade_stats_by_type`。这样做是为了确保 `analyze_optimization_results` 函数能获取到该次回测运行的最终统计。

2.  **在 `analyze_optimization_results` 函数中**：
    *   在处理每个参数组合的 `strategy_instance` 时，从策略实例中获取 `final_trade_stats_by_type` 字典。
    *   将这个 `trade_stats_by_type` 字典存入 `processed_results`，与夏普率、收益率等指标一起。
    *   在打印“最优参数组合”的详细信息时，从最佳结果的 `trade_stats_by_type` 中提取趋势交易和区间交易的次数，并以 "执行交易统计: 趋势 X笔, 区间 Y笔" 的格式显示出来。
...
class AShareETFStrategy(bt.Strategy):
    ...
    def log(self, txt, dt=None, data=None):
        ...
        return # 当前日志功能被禁用。若要启用，请注释掉或删除此行。
    ...
    def __init__(self):
        ...
        # (创建一个叫 `self.trade_details_by_type` 的详细记录本，区分趋势交易、区间交易和未知类型交易的各种统计数据，比如总盈亏、交易次数、成功次数、失败次数等。)
        self.final_trade_stats_by_type = {}
        # (准备一个空本子 `self.final_trade_stats_by_type`，在策略跑完一次后，会把 `self.trade_details_by_type` 的最终结果存到这里，供分析函数使用。)
    ...
    def notify_trade(self, trade):
        ...
        # 获取交易类型，用于统计
        trade_type = self.position_types.get(data_name, 'unknown')
        # 从 `self.position_types` 本子里查一下这笔交易当初是按什么类型（'trend' 或 'range'）开的仓。如果查不到，就当是 'unknown'（未知类型）。

        self.log(
            f'交易利润 for {data_name}, 毛利 {trade.pnl:.2f}, 净利 {trade.pnlcomm:.2f}, 持仓类型: {trade_type}', data=trade.data)
        # 记录交易的盈利情况 (毛利润 `trade.pnl` 和扣除佣金后的净利润 `trade.pnlcomm`) 以及该持仓的类型。
        # (记个日志说这笔交易赚了还是亏了多少钱（没算手续费的和算了手续费的），以及当初是按什么类型（趋势/区间）买的。)

        # 更新交易类型统计
        if trade_type not in self.trade_details_by_type: # 以防万一类型不在字典中
            trade_type = 'unknown'
        # (再检查一下，确保 `trade_type` 是我们 `trade_details_by_type` 本子里有的类型，如果没有就归为 'unknown'。)

        self.trade_details_by_type[trade_type]['pnl'] += trade.pnlcomm
        # 把这笔交易的净盈亏加到对应类型的总盈亏里。
        self.trade_details_by_type[trade_type]['count'] += 1
        # 把对应类型的交易次数加1。

        if trade.pnlcomm > 0:
            # 如果这笔交易是赚钱的。
            self.trade_details_by_type[trade_type]['wins'] += 1
            # 对应类型的盈利次数加1。
            self.trade_details_by_type[trade_type]['total_pnl_wins'] += trade.pnlcomm
            # 把盈利金额加到对应类型的总盈利金额里。
        elif trade.pnlcomm < 0:
            # 如果这笔交易是亏钱的。
            self.trade_details_by_type[trade_type]['losses'] += 1
            # 对应类型的亏损次数加1。
            self.trade_details_by_type[trade_type]['total_pnl_losses'] += trade.pnlcomm
            # 把亏损金额（是个负数）加到对应类型的总亏损金额里。
        ...
        for i, d_obj in enumerate(self.datas):
            ...
            try:
                ...
                if is_trend_up:
                    ...
                    # 将市场状态设置为 'TREND_UP'。
                    # (就把市场状态改成 'TREND_UP'。)
                elif is_range_confirmed: # 不再检查 self.p.etf_type，因为策略动态决定
                    # 如果判断为区间震荡。
                    # (如果是区间震荡。)
            ...
            # (如果当前市场是上升趋势，就检查趋势买入信号，并计算相应的风险。)
            ...
            # (如果当前市场是区间震荡，就检查区间买入信号，并计算相应的风险。)
    ...
    def stop(self):
        # 定义 `stop` 方法，在回测结束时由Backtrader调用一次。
        # (定义一个名叫 `stop` 的函数，当整个回测跑完之后，`backtrader` 会最后调用一次这个函数。)
        self.log(f'期末总资金: {self.broker.getvalue():.2f}',
                 dt=self.datas[0].datetime.date(0) if len(self.datas[0]) else None)
        # 记录期末总资金和最后的日期（如果数据存在）。
        # (记个日志，显示回测结束时账户总共有多少钱，以及最后的日期是什么时候。)

        # 将本次运行的交易类型统计数据赋值给 final_trade_stats_by_type
        # 这样，在参数优化时，每个策略实例的这个属性就能被 analyze_optimization_results 函数获取
        self.final_trade_stats_by_type = self.trade_details_by_type.copy()
        # (把这次回测过程中积累的 `trade_details_by_type`（各种交易类型的统计数据）复制一份存到 `self.final_trade_stats_by_type` 里。
        #  用 `.copy()` 是为了防止意外修改，虽然这里可能不是必须的，但算是个好习惯。
        #  这样，外面分析优化结果的函数就能拿到这次跑完后的最终统计了。)

        # 可选：在这里打印本次运行的交易类型统计，用于调试或单次回测查看
        # self.log('策略内部交易类型统计 (来自 stop 方法):')
        # for trade_type, stats in self.final_trade_stats_by_type.items():
        #     self.log(f"  类型: {trade_type}, 总盈亏: {stats['pnl']:.2f}, 次数: {stats['count']}, "
        #              f"盈利次数: {stats['wins']}, 亏损次数: {stats['losses']}, "
        #              f"总盈利额: {stats['total_pnl_wins']:.2f}, 总亏损额: {stats['total_pnl_losses']:.2f}")
        # (如果想在每次策略跑完（包括单次回测或者优化中的某一次）都看看交易类型的统计，可以把上面这几行注释打开。它会把趋势交易、区间交易等各自的盈亏、次数等信息打印出来。)
...
def analyze_optimization_results(results):
    ...
    for strat_list in results:
        ...
        # 定义在优化过程中实际使用的参数名称列表，以及那些虽然固定但仍希望在结果中看到的参数。
        # (列出我们这次优化主要关心的那些参数的名字，以及那些虽然固定不变但我们也想在结果里看到的参数。)
        ...
        try:
            ...
            # 新增：从策略实例中获取最终的交易类型统计数据
            trade_stats_by_type = getattr(strategy_instance, 'final_trade_stats_by_type', {
                'trend': {'count': 0}, 'range': {'count': 0}, 'unknown': {'count': 0}
            })
            # (尝试从跑完的策略实例 `strategy_instance` 中获取 `final_trade_stats_by_type` 这个本子里的内容。
            #  如果因为某些原因找不到这个本子，就默认趋势、区间、未知类型的交易次数都是0，避免程序出错。)

            processed_results.append({
                # 'instance': strategy_instance, # 保留完整实例可能占用大量内存，特别是优化时。通常只保留必要数据。
                # (如果优化的组合特别多，把每个策略实例都存下来可能会用很多电脑内存，所以这里把它注释掉了，我们只存需要的数据。)
                'params_dict': current_params_dict, # 当前参数组合的字典
                # (把这次跑的参数 `current_params_dict` 存起来。)
                'sharpe': sharpe,                   # 夏普比率
                # (把算出来的夏普率 `sharpe` 存起来。)
                'return': total_return,             # 总收益率
                # (把总收益率 `total_return` 存起来。)
                'drawdown': max_drawdown,           # 最大回撤
                # (把最大回撤 `max_drawdown` 存起来。)
                'trade_stats_by_type': trade_stats_by_type # 新增：存储交易类型统计
                # (把这次跑完后的交易类型统计 `trade_stats_by_type` 也存起来。)
            })
            successful_runs += 1
            # 成功提取结果的运行次数加1。
            ...
            # self.log(f"参数组 {params_str} 的分析器结果提取失败: {e}") # 可以取消注释用于调试
            # (如果想看具体是哪个参数组合出错了，可以把上面这行取消注释。)
            pass
            # 忽略此错误，继续处理下一个参数组合。
            # (那就跳过这个参数组合，继续处理其他的。)
        except Exception as e:
            ...
            # (如果发生了其他类型的错误。)
            # self.log(f"处理参数组 {params_str} 时发生未知错误: {e}") # 可以取消注释用于调试
            import traceback
            # 导入 `traceback` 模块。
            # (引入 `traceback` 工具。)
            # traceback.print_exc() # 可以取消注释用于详细错误调试
            pass
            # 忽略此错误，继续处理。
            # (也跳过这个参数组合，继续处理其他的。)
...
if __name__ == '__main__':
    ...
    data_folder = r'D:\\BT2025\\datas\\' # Windows系统路径示例，请根据您的实际路径修改
    # (这里直接写死了数据文件夹的路径在 D盘的 BT2025 文件夹下的 datas 文件夹里。您需要根据您的实际情况修改这个路径。)
    # 例如，如果您在Linux或macOS上，路径可能是 /path/to/your/datas/
    # data_folder = '/path/to/your/BT2025/datas/' # Linux/macOS 示例路径
    ...
    if optimize:
        ...
        # 打印参数优化设置的标题。
        # (打一个标题"参数优化设置"。)
        # 注意：etf_type 不再作为优化参数，策略会动态判断市场并执行趋势或区间逻辑
        # print(f"  etf_type: (策略动态判断)")
        print(f"  EMA中期 (ema_medium_period): {list(ema_medium_range)}")
        # 打印EMA中期均线周期的优化范围。
        # (显示中期EMA均线周期会尝试的值。)
        print(f"  EMA长期 (ema_long_period): {list(ema_long_range)}")
        # 打印EMA长期均线周期的优化范围。
        # (显示长期EMA均线周期会尝试的值。)
        print(f"  布林带周期 (bbands_period): {list(bbands_period_range)}")
        # 打印布林带周期的优化范围。
        # (显示布林带周期会尝试的值。)
        print(f"  布林带标准差 (bbands_devfactor): {bbands_dev_range}")
        # 打印布林带标准差倍数的优化范围。
        # (显示布林带标准差倍数会尝试的值。)
        print(f"  趋势止损ATR倍数 (trend_stop_loss_atr_mult): {trend_sl_atr_mult_range}")
        # 打印趋势止损ATR倍数的优化范围。
        # (显示趋势止损ATR倍数会尝试的值。)
        print(f"  区间止损缓冲 (range_stop_loss_buffer): {range_sl_buffer_range}") # 如果要优化此参数，取消optstrategy中的注释
        # (显示区间止损缓冲会尝试的值，如果这个参数也要优化，记得把下面 `optstrategy` 里的注释去掉。)
        print(f"  趋势交易风险 (risk_per_trade_trend): {risk_trend_range}") # 如果要优化此参数，取消optstrategy中的注释
        # (显示趋势交易风险比例会尝试的值，如果这个参数也要优化，记得把下面 `optstrategy` 里的注释去掉。)
        print(f"  区间交易风险 (risk_per_trade_range): {risk_range_range}") # 如果要优化此参数，取消optstrategy中的注释
        # (显示区间交易风险比例会尝试的值，如果这个参数也要优化，记得把下面 `optstrategy` 里的注释去掉。)
        print('-' * 50)
        # 打印分隔线。
        # (打一条分隔线。)
        ...
        cerebro.optstrategy(AShareETFStrategy,
                            # etf_type=['trend', 'range'], # 此参数已从策略决策逻辑中解耦，不再直接通过优化控制交易类型选择
        ...
        # - `etf_type` 参数不再通过优化直接控制，因为策略会动态判断市场状态。
        # (把我们的 `AShareETFStrategy` 策略加到 `cerebro` 里进行优化。告诉它 `ema_medium_period` 尝试 `ema_medium_range` 里的值，以此类推。
        #  注意：如果想让 `range_stop_loss_buffer`, `risk_per_trade_trend`, `risk_per_trade_range` 这些参数也参与优化，
        #  需要确保它们在上面的 `optstrategy` 调用中没有被注释掉。)
        ...
        if best_result:
            ...
            row_format_str = '{etf_type:<8} {ema_medium_period:<8.0f} {ema_long_period:<8.0f} {bbands_period:<8.0f} {bbands_devfactor:<8.1f} {trend_stop_loss_atr_mult:<8.1f} {range_stop_loss_buffer:<8.4f} {risk_per_trade_trend:<10.3%} {risk_per_trade_range:<10.3%} {sharpe:<10.4f} {return_val:<10.2%} {drawdown_val:<10.2%} {score:<10.4f}'
            # 定义结果表格每行数据的格式字符串。注意风险、收益、回撤使用百分号格式化。
            # (规定一下表格每一行数据的格式。注意风险、收益、回撤这里用了 `{:.n%}` 的格式，可以直接显示百分比。)

            num_cols = 13
            # 表格的列数。
            # (表格总共有13列。)
            col_widths = [8, 8, 8, 8, 8, 8, 8, 10, 10, 10, 10, 10, 10] # 确保与header_format_str和row_format_str的字段数匹配
            # 每列的宽度列表。
            # (每一列的宽度分别是多少。)
            total_width = sum(col_widths) + (len(col_widths) -1) *1 # 考虑列之间的空格
            # 计算表格总宽度。
            # (算一下表格总共多宽，这里假设列之间有一个空格。)

            print(f'\n{(" 参数优化结果 (按得分排序, 最多显示20组) "):=^{total_width}}')
            # 打印表格标题，使其在总宽度内居中。
            # (打一个表格的标题"参数优化结果 (按得分排序, 最多显示20组)"，让它在表格中间。)
            print(header_format_str.format('ETF类型', 'EMA中', 'EMA长', 'BB周期',
                  'BB标差', 'ATR止损', '区间SL', '趋势风险', '区间风险', '夏普', '总收益', '最大回撤', '得分'))
            # 打印表头。
            ...
            for res_data in all_scored_results[:min(20, len(all_scored_results))]:
                ...
                print(row_format_str.format(
                    etf_type=p_dict.get('etf_type', 'N/A'), # etf_type 会是默认值 'trend'
                    ...
                    risk_per_trade_trend=p_dict.get( # 直接传入小数，格式化字符串处理百分比
                        'risk_per_trade_trend', 0.0),
                    risk_per_trade_range=p_dict.get( # 直接传入小数，格式化字符串处理百分比
                        'risk_per_trade_range', 0.0),
                    sharpe=res_data.get('sharpe', 0.0),
                    return_val=res_data.get('return', 0.0), # 直接传入小数
                    drawdown_val=res_data.get('drawdown', 0.0), # 直接传入小数
            ...
            # 打印最优参数组合的标题。
            # (打一个标题"最优参数组合"。)
            best_trade_stats = best_result.get('trade_stats_by_type', {})
            # 从最好的结果里获取交易类型的统计数据。如果拿不到，就用一个默认的空统计。
            trend_trades_count = best_trade_stats.get('trend', {}).get('count', 0)
            # 获取趋势交易的次数，如果趋势交易没数据或者次数没记录，就当是0。
            range_trades_count = best_trade_stats.get('range', {}).get('count', 0)
            # 获取区间交易的次数。
            unknown_trades_count = best_trade_stats.get('unknown', {}).get('count', 0)
            # 获取未知类型交易的次数。

            trade_summary_parts = []
            # 准备一个列表，用来放交易统计的文字片段。
            if trend_trades_count > 0:
                trade_summary_parts.append(f"趋势 {trend_trades_count}笔")
            # 如果有趋势交易，就加上 "趋势 X笔"。
            if range_trades_count > 0:
                trade_summary_parts.append(f"区间 {range_trades_count}笔")
            # 如果有区间交易，就加上 "区间 Y笔"。
            if unknown_trades_count > 0:
                trade_summary_parts.append(f"未知 {unknown_trades_count}笔")
            # 如果有未知类型交易，也加上。
            trade_type_summary = ", ".join(trade_summary_parts) if trade_summary_parts else "无有效交易"
            # 把这些文字片段用逗号连起来。如果没有有效交易，就显示 "无有效交易"。

            print(f"{'执行交易统计':<25}: {trade_type_summary}")
            # 打印我们新加的这行，显示具体执行了多少笔趋势交易和区间交易。
            print(f"{'ETF类型(参数)':<25}: {best_params_dict.get('etf_type', 'N/A')}") # 这个是策略参数，非动态结果
            # (显示策略参数 `etf_type` 的值，这个值是固定的或者优化时设定的，不是根据交易动态变的。)
    ...
    # (如果 `optimize` 开关是关着的，就执行这里，只用一组固定的参数跑一次回测。)
        ...
        strategy_run_params = dict(
            # etf_type='trend', # 此参数在策略内部不再强行决定逻辑，而是动态判断
            ema_medium_period=60,
            ema_long_period=120,
            bbands_period=20,
            bbands_devfactor=2.0,
        ...
        if results:  # 检查 results 是否非空 (单次回测时，results是包含一个策略实例的列表)
            ...
            # 打印交易类型统计
            print("\n--- 交易类型统计 (来自 final_trade_stats_by_type) ---")
            # (打一个子标题，显示交易类型的统计。)
            trade_stats = getattr(strat_instance, 'final_trade_stats_by_type', {})
            # (从跑完的策略实例里拿出 `final_trade_stats_by_type` 这个本子。)
            if trade_stats:
                # 如果这个本子不是空的。
                for trade_type, stats_dict in trade_stats.items():
                    # (一个一个地看里面的每种交易类型（比如趋势、区间）。)
                    print(f"  类型: {trade_type}")
                    # (打出类型名字。)
                    for stat_name, stat_value in stats_dict.items():
                        # (再看这个类型下面的具体统计项，比如次数、总盈亏等。)
        ...
        if not optimize:  # 确保 optimize 确实为 False
            ...
            try:
                ...
                if cerebro.datas:
                    ...
                    # (看看 `cerebro` 大脑里有没有数据。)
                    # data_to_plot_name = cerebro.datas[0]._name # 旧的只绘制第一个
