            ...
            # If no position, check for entry conditions
                ...
                if entry_signal and stop_loss_price_calc is not None and entry_price_calc > stop_loss_price_calc:
                    ...
                    if size > 0:
                        ...
                        self.log(
                            f'BUY BRACKET SIGNAL: {data_name}, Size: {size}, Entry: ~{entry_price_calc:.2f}, SL: {stop_loss_price_calc:.2f}, TP: {take_profit_price_calc if take_profit_price_calc else "N/A"}, Type: {potential_position_type}', data=d)
                        ...
                        if order_buy:
                            ...
                            self.orders[d] = order_buy[0] # Track entry order
                            self.stop_loss_prices[d] = stop_loss_price_calc # Store for info
                            self.take_profit_prices[d] = take_profit_price_calc # Store for info
