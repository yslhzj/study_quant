...
def analyze_optimization_results(results):
    ...
    for i, strategy_instance in enumerate(results):
        ...
        try:
            ...
            # 假设 .p 有这些属性
            params_repr = f"{params.ema_medium_period}, {params.ema_long_period}, {params.bbands_period}, {params.bbands_devfactor}"
            # Assume .p has these attributes
            if not sharpe_analysis or 'sharperatio' not in sharpe_analysis:
                print(
                    f"警告: 参数组 {params_repr} 的夏普比率分析结果不完整或为None，跳过。Sharpe: {sharpe_analysis}")
            ...
            if not returns_analysis or 'rtot' not in returns_analysis:
                print(
                    f"警告: 参数组 {params_repr} 的收益率分析结果不完整，跳过。Returns: {returns_analysis}")
            ...
            if not drawdown_analysis or 'max' not in drawdown_analysis or 'drawdown' not in drawdown_analysis.max:
                print(
                    f"警告: 参数组 {params_repr} 的最大回撤分析结果不完整，跳过。Drawdown: {drawdown_analysis}")
...
if __name__ == '__main__':
    ...
    if optimize:
        ...
        print_optimization_settings(
            optimize, ema_medium_range, ema_long_range, bbands_period_range, bbands_dev_range)
