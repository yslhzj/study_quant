...
def print_optimization_summary(best_result, all_scored_results):
    ...
    # 打印排序后的详细结果列表
    # Print the sorted detailed results list
    # 增加序号列，调整宽度
    print('\n{:=^105}'.format(' 参数优化结果 (按得分排序) '))  # 增加宽度
    ...
    for idx, res_data in enumerate(all_scored_results, start=1):  # 使用 enumerate 获取序号
        # 遍历排序后的结果。 (Iterate through the sorted results.)
        p = res_data['params']
        # 获取参数。 (Get the parameters.)
        # 增加序号列 idx
        print('{:<6} {:<12} {:<12} {:<12} {:<10.1f} {:<12.4f} {:<12.2f} {:<12.2f} {:<12.4f}'.format(
            idx, p.ema_medium_period, p.ema_long_period, p.bbands_period, p.bbands_devfactor,
            res_data['sharpe'], res_data['return'] * 100,
            res_data['drawdown'] * 100, res_data['score']
        ))
        # 打印格式化的结果行。 (Print the formatted result row.)

    # 打印最优参数总结
    # Print the best parameter summary
    if best_result:
...
def run_backtest():
    ...
    if optimize:
        ...
        # 注意：需要导入 numpy 才能正确显示浮点数范围
        # Note: Need to import numpy to display float range correctly
        try:
            import numpy as np
        print(f"  bbands_devfactor: {list(np.round(bbands_dev_range, 2))}")
        except ImportError:
            print(f"  bbands_devfactor: {bbands_dev_range} (需要numpy以更好显示)")
        print('-' * 50)
        # --- [恢复] 打印优化设置结束 ---
        ...
        print('开始参数优化运行...')  # [恢复] 开始运行提示
        start_time = time.time()  # [恢复] 记录开始时间
        results = cerebro.run(maxcpus=10)
        end_time = time.time()  # [恢复] 记录结束时间
        print("--- 参数优化完成 ---")
        ...
        # 使用辅助函数打印优化结果 (包括详细列表和最优组合)
        # Use helper function to print optimization results (including detailed list and best combo)
