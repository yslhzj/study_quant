...
def run_backtest():
    ...
    # 检查是否成功加载了任何数据 (恢复检查)
    # Check if any data was loaded successfully (restore check)
    ...
    print(f"\n总共加载了 {loaded_data_count} 个数据源。") # 恢复原始风格
    ...
    cerebro.broker.setcommission(commission=commission_rate, stocklike=True) # 恢复 stocklike=True
    ...
    if optimize:
        ...
        # 添加优化所需的分析器 (提前添加)
        # Add analyzers required for optimization (add earlier)
        cerebro.addanalyzer(bt.analyzers.SharpeRatio,
                            _name='sharpe_ratio', riskfreerate=0.0, timeframe=bt.TimeFrame.Days) # 恢复 timeframe
        ...
        # TradeAnalyzer 在优化时通常不是必须的，但如果 analyze_optimization_results 需要它，则应添加
        ...
        # 运行优化过程。 (Run the optimization process.)

        end_time = time.time() # 恢复计时
        ...
        # --- 优化结果分析与打印 (Optimization Result Analysis and Printing) ---
        ...
        if best_result:
            ...
            print('\n{:=^50}'.format(' 最优参数组合 ')) # 恢复原始风格
            # 打印最优参数组合标题。 (Print the best parameter combination title.)
            best_params = best_result['params']
            # 获取最佳参数。 (Get the best parameters.)
            print('{:<20}: {}'.format('EMA中期', best_params.ema_medium_period)) # 恢复原始风格
            # 打印最佳中期EMA。 (Print the best medium EMA.)
            print('{:<20}: {}'.format('EMA长期', best_params.ema_long_period)) # 恢复原始风格
            # 打印最佳长期EMA。 (Print the best long EMA.)
            print('{:<20}: {}'.format('布林带周期', best_params.bbands_period)) # 恢复原始风格
            # 打印最佳布林带周期。 (Print the best Bollinger Bands period.)
            # 注意：原始日志显示的是 np.float64，这里格式化为.1f应该可以
            print('{:<20}: {:.1f}'.format(
                '布林带标准差', best_params.bbands_devfactor)) # 恢复原始风格
            # 打印最佳布林带标准差。 (Print the best Bollinger Bands deviation factor.)
            print('{:<20}: {:.4f}'.format('夏普比率', best_result['sharpe'])) # 恢复原始风格
            # 打印最佳夏普比率。 (Print the best Sharpe Ratio.)
            print('{:<20}: {:.2f}%'.format(
                '总收益率', best_result['return'] * 100)) # 恢复原始风格
            # 打印最佳总收益率。 (Print the best total return.)
            print('{:<20}: {:.2f}%'.format(
                '最大回撤', best_result['drawdown'] * 100)) # 恢复原始风格
            # 打印最佳最大回撤。 (Print the max drawdown.)
            print('{:<20}: {:.4f}'.format('得分', best_result['score'])) # 恢复原始风格
        ...
        # 添加观察器 (因为 optimize=False, stdstats=True, 会自动添加一些，这里可以补充)
        # Add observers (Since optimize=False, stdstats=True, some are added automatically, supplement here)
        # cerebro.addobserver(bt.observers.Broker) # stdstats=True 时通常已添加
        # cerebro.addobserver(bt.observers.Trades) # stdstats=True 时通常已添加
        # cerebro.addobserver(bt.observers.BuySell) # stdstats=True 时通常已添加
        cerebro.addobserver(bt.observers.DrawDown) # 可以额外添加
        # 添加回撤观察器。 (Add DrawDown observer.)

        # 运行回测
        # Run the backtest
        print('开始单次回测运行...') # 恢复原始风格
        # 打印开始运行单次回测的提示。 (Print a message indicating the start of the single backtest run.)
        print('期初总资金: %.2f' % cerebro.broker.getvalue()) # 恢复原始风格
        ...
        end_time = time.time() # 恢复计时
        # 记录结束时间。 (Record the end time.)
        final_value = cerebro.broker.getvalue() # 获取最终价值
        # 获取最终投资组合价值。 (Get the final portfolio value.)

        print('期末总资金: %.2f' % final_value) # 恢复原始风格
        # 打印期末资金。 (Print the final portfolio value.)
        print('回测总用时: {:.2f}秒'.format(end_time - start_time)) # 恢复原始风格
        # 打印回测耗时。 (Print the backtest duration.)

        # --- 结果打印与分析 (Result Printing and Analysis) ---
        # print("--- 回测结果分析 ---") # 可以用下面的标题代替
        # 打印回测结果分析的标题。 (Print the title for backtest result analysis.)
        print(f'\n初始组合价值 (Initial Portfolio Value): {initial_cash:.2f}') # 恢复格式
        # 打印初始投资组合价值。 (Print the initial portfolio value.)
        print(f'\n最终组合价值 (Final Portfolio Value):   {final_value:.2f}') # 恢复格式
        # 打印最终投资组合价值。 (Print the final portfolio value.)
        pnl = final_value - initial_cash
        # 计算净利润/亏损。 (Calculate the net profit/loss.)
        print(f'\n净利润/亏损 (Net Profit/Loss):       {pnl:.2f}') # 恢复格式
        # 打印净利润/亏损。 (Print the net profit/loss.)
        ...
        # 对于单次回测，可以考虑绘制图表 (恢复原始逻辑)
        # For single backtest runs, consider plotting (restore original logic)
        if not optimize: # 仅在非优化模式下绘图 (确保这部分在 if not optimize: 内部)
             try:
                 print("\n尝试绘制图表...") # 恢复原始风格
                 ...
                 figs = cerebro.plot(style='candlestick', barup='red', bardown='green',
                                     numfigs=1, volume=True, iplot=True) # iplot=True 恢复交互式绘图
                 # 尝试绘制K线图。 (Attempt to plot the candlestick chart.)
                 print("图表窗口可能已打开。") # 恢复原始风格
             ...
             except Exception as e: # 修复 linter error: except 需要在 try 之后且有内容
                 print(f"\n绘制图表时出错: {e}") # 恢复原始风格
