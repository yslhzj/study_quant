...
def run_backtest():
    ...
    # 注意：原始日志显示 bbands_dev_range 为 np.float64 列表, 但直接用 float 列表通常也兼容
    ...
    if optimize:
        ...
        if best_result:
            ...
            print('{:<20}: {}'.format(
                'EMA中期', best_params.ema_medium_period))  # 恢复原始风格
            # 打印最佳中期EMA。 (Print the best medium EMA.)
            print('{:<20}: {}'.format(
                'EMA长期', best_params.ema_long_period))  # 恢复原始风格
            # 打印最佳长期EMA。 (Print the best long EMA.)
            print('{:<20}: {}'.format(
                '布林带周期', best_params.bbands_period))  # 恢复原始风格
            # 打印最佳布林带周期。 (Print the best Bollinger Bands period.)
            ...
            print('{:<20}: {:.4f}'.format(
                '夏普比率', best_result['sharpe']))  # 恢复原始风格
            # 打印最佳夏普比率。 (Print the best Sharpe Ratio.)
            print('{:<20}: {:.2f}%'.format(
        ...
        # 恢复原始风格 (根据 stdstats 联动)
        print(f"Observer 图表: {'关闭' if optimize else '开启'}")
        ...
        # 使用普通round
        print(f"  bbands_devfactor: {[round(x, 2) for x in bbands_dev_range]}")
        ...
        # 打印回测结果分析的标题。 (Print the title for backtest result analysis.)
        # 恢复格式
        print(f'\n初始组合价值 (Initial Portfolio Value): {initial_cash:.2f}')
        # 打印初始投资组合价值。 (Print the initial portfolio value.)
        ...
        if not optimize:  # 仅在非优化模式下绘图 (确保这部分在 if not optimize: 内部)
            try:
                ...
                print("请确保已安装matplotlib且图形环境配置正确。")  # 恢复原始风格
