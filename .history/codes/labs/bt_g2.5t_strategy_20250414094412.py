...
class AShareETFStrategy(bt.Strategy):
    ...
    def notify_order(self, order):
        ...
        if order.status == order.Submitted:
            ...
            self.log(
                f'Order {order.ref} SUBMITTED for {data_name}', data=order_data)
        ...
        if order.status == order.Accepted:
            ...
            self.log(
                f'Order {order.ref} ACCEPTED for {data_name}', data=order_data)
        ...
        if order.status == order.Partial:
            ...
            # 记录执行K线索引 (Record execution bar index)
            ...
            # Order canceled
            self.log(
                f'Order {order.ref} CANCELED for {data_name}', data=order_data)
            ...
            # Order expired
            self.log(
                f'Order {order.ref} EXPIRED for {data_name}', data=order_data)
            ...
            # Margin call
            self.log(
                f'Order {order.ref} MARGIN for {data_name}', data=order_data)
            ...
            # Order rejected
            self.log(
                f'Order {order.ref} REJECTED for {data_name}', data=order_data)
