...
# Note: Specific logic for SL/TP trigger detection might need refinement based on how they are implemented (e.g., separate orders, price monitoring)
...
class JSONFormatter(logging.Formatter):
    ...
    def format(self, record):
        ...
        # Assume record.msg is already a dictionary prepared by log_event
        log_object = record.msg
        ...
        try:
            ...
            error_log = {
                ...
                # Log the problematic record as string
                "original_record": str(log_object)
            }
...
class AShareETFStrategy(bt.Strategy):
    ...
    params = (
        # 短期EMA周期 (Short-term EMA period)
        ('ema_short_period', 12),
        # 中期EMA周期 (Medium-term EMA period)
        ('ema_medium_period', 26),
        # 长期EMA周期 (Long-term EMA period)
        ('ema_long_period', 52),
        # 布林带周期 (Bollinger Bands period)
        ('bbands_period', 20),
        # 布林带标准差倍数 (Bollinger Bands deviation factor)
        ('bbands_devfactor', 2.0),
        ('rsi_period', 14),                           # RSI周期 (RSI period)
        # RSI超买阈值 (RSI overbought threshold)
        ('rsi_overbought', 70),
        # RSI超卖阈值 (RSI oversold threshold)
        ('rsi_oversold', 30),
        # MACD快线周期 (MACD fast period)
        ('macd_fast_period', 12),
        # MACD慢线周期 (MACD slow period)
        ('macd_slow_period', 26),
        # MACD信号线周期 (MACD signal period)
        ('macd_signal_period', 9),
        ('atr_period', 14),                           # ATR周期 (ATR period)
        # 基于ATR的止损倍数 (ATR-based stop loss multiplier)
        ('stop_loss_atr_multiplier', 2.0),
        # 基于ATR的止盈倍数 (ATR-based take profit multiplier)
        ('take_profit_atr_multiplier', 3.0),
        # 每笔交易风险百分比 (Risk per trade percentage)
        ('risk_per_trade_percent', 0.01),
        # 最大总风险百分比 (Maximum total risk percentage)
        ('max_total_risk_percent', 0.1),
        # 是否启用动态风险调整 (Enable dynamic risk adjustment)
        ('enable_dynamic_risk_adjustment', True),
        # 最大回撤限制百分比 (Maximum drawdown limit percentage)
        ('max_drawdown_limit_percent', 0.15),
        # 连续亏损次数限制 (Consecutive losses limit)
        ('consecutive_losses_limit', 5),
        # 风险降低因子 (Risk reduction factor)
        ('risk_reduction_factor', 0.5),
        # 交易暂停持续期（K线数） (Trading halt duration in bars)
        ('trading_halt_duration_bars', 10),
        # 订单重试次数 (Order retry attempts)
        ('order_retry_attempts', 3),
        # 订单重试延迟（秒） (Order retry delay in seconds)
        ('order_retry_delay_seconds', 5),
        # 是否启用日志记录 (Enable logging)
        ('enable_logging', True),
        # 日志文件路径 (Log file path) - FR-LOG-008
        ('log_path', 'logs'),
        # 日志文件名模板 - FR-LOG-009
        ('log_filename_pattern',
         '{strategy_name}_{data_name}_{timestamp:%Y%m%d_%H%M%S}_{pid}.jsonl'),
        # 日志级别 ('INFO', 'DEBUG') - FR-LOG-010
        ('log_level', 'INFO'),
        ('log_async_write', True),                    # 是否异步写入日志 - FR-LOG-011
        # 日志轮转大小(MB) - FR-LOG-012 (optional)
        ('log_rotation_size_mb', 100),
        # 日志轮转备份数量 - FR-LOG-012 (optional)
    ...
    def _setup_logger(self):
        ...
        data_name_safe = self.datas[0]._name.replace(
            '*', '_').replace(':', '_')  # Basic sanitization
        ...
        if self.p.log_async_write:
            ...
            self._log_queue_listener = QueueListener(
    ...
    def log_event(self, event_type: str, data_feed: bt.DataBase = None, **kwargs):
        ...
        try:
            ...
            # SL/TP might be order updates
            if event_type in [ORDER_STATUS_UPDATE, SL_TP_TRIGGERED]:
                # Expect order_ref in kwargs
                ...
                if order_ref:
                    ...
                    trade_id = self.order_ref_to_trade_id_map.get(
                        order_ref, 'ORPHANED_ORDER_EVENT')
                    log_entry['trade_cycle_id'] = trade_id
                    # Ensure order_ref is logged
                    log_entry['order_ref'] = order_ref

            elif event_type == TRADE_CLOSED:
                # Expect list of order refs in kwargs
                trade_orders_refs = kwargs.get('trade_orders_refs')
                trade_id = 'ORPHANED_TRADE_EVENT'  # Default
                if trade_orders_refs:
                    # Log refs for debugging
                    log_entry['trade_orders_refs'] = trade_orders_refs
            ...
            print(
                f"{datetime.now(timezone.utc).isoformat()} [LogSystemError]: Failed to log event - {e}")
    ...
    def __init__(self, **kwargs):  # 接受 **kwargs 以兼容 addstrategy 传递的参数
        ...
        self.ema_short = bt.indicators.ExponentialMovingAverage(
            self.datas[0], period=self.p.ema_short_period)
        ...
        self.ema_medium = bt.indicators.ExponentialMovingAverage(
            self.datas[0], period=self.p.ema_medium_period)
        ...
        self.ema_long = bt.indicators.ExponentialMovingAverage(
            self.datas[0], period=self.p.ema_long_period)
        # 计算长期指数移动平均线（EMA）。 (Calculate the long-term Exponential Moving Average (EMA).)
        self.bbands = bt.indicators.BollingerBands(
            self.datas[0], period=self.p.bbands_period, devfactor=self.p.bbands_devfactor)
        ...
        self.macd = bt.indicators.MACD(self.datas[0], period_me1=self.p.macd_fast_period,
        ...
        # 交易暂停至第几根K线 (Trading halted until bar number)
        self.trading_halted_until_bar = 0
        self.order_retry_counts = defaultdict(
            int)  # 订单重试计数器 (Order retry counter)

        # 用于跟踪订单和交易。
        # For tracking orders and trades.
        self.active_orders = {}  # 活跃订单字典 (Active orders dictionary)
        # 当前交易周期的唯一ID (Unique ID for the current trade cycle) - FR-ID-001
        self.current_trade_id = None
        # 订单引用到交易周期ID的映射 (Mapping from order reference to trade cycle ID) - FR-ID-003
    ...
    def notify_order(self, order):
        ...
        if order.executed.price:
            order_log_data["exec_details"] = {
                "avg_price": order.executed.price}  # Example

        # FR-ID-003: Retrieve trade_cycle_id using the map (will be added by log_event)
        self.log_event(event_type=ORDER_STATUS_UPDATE,
                       data_feed=order.data, **order_log_data)
        # ---------------------------------
    ...
    def notify_trade(self, trade):
        ...
        if trade.isclosed:
            trade_log_data = {
                ...
                # Requires trade.long attribute if available
                "position_type": 'Long' if trade.long else 'Short'
            }
            self.log_event(event_type=TRADE_CLOSED,
                           data_feed=trade.data, **trade_log_data)
    ...
    def notify_cashvalue(self, cash, value):
        ...
        current_drawdown = (self.high_water_mark - value) / \
        ...
        if self.p.enable_dynamic_risk_adjustment and current_drawdown > self.p.max_drawdown_limit_percent:
            # Avoid logging every bar during halt
            if self.trading_halted_until_bar <= len(self):
                ...
                self.trading_halted_until_bar = len(
                    self) + self.p.trading_halt_duration_bars
    ...
    def _calculate_trade_size(self, data_close_price, entry_price, stop_loss_price, risk_per_trade_percent, data=None):
        ...
        risk_amount_per_trade = account_value * \
            risk_per_trade_percent * self.current_risk_multiplier
        ...
        if price_risk_per_share <= 0:
            ...
            calc_log_data["adjustment_reasons"].append(
                "Zero or negative price risk per share")
            self.log_event(event_type=ORDER_CALCULATION,
                           data_feed=data, **calc_log_data)
        ...
        if size_raw <= 0:
            calc_log_data["size_final"] = 0
            calc_log_data["adjustment_reasons"].append(
                "Calculated raw size is non-positive")
            self.log_event(event_type=ORDER_CALCULATION,
                           data_feed=data, **calc_log_data)
            return 0
        ...
        required_margin = comminfo.getmargin(
            entry_price) * size_raw  # Simplified margin calc
        ...
        if potential_new_total_risk > max_allowed_total_risk:
            calc_log_data["adjustment_reasons"].append(
                "Maximum total portfolio risk exceeded")
        ...
        if available_cash < required_value + estimated_commission:  # Simple check
            calc_log_data["adjustment_reasons"].append(
                "Insufficient cash for calculated size + commission")
        ...
        calc_log_data["trade_cycle_id"] = getattr(
            self, 'current_trade_id', None)
        self.log_event(event_type=ORDER_CALCULATION,
                       data_feed=data, **calc_log_data)
    ...
    def _place_order(self, data, order_type, size, price=None, exectype=None, **kwargs):
        ...
        trade_cycle_id = getattr(
            self, 'current_trade_id', None)  # Get the current ID
        ...
        # Add any extra arguments like stop_loss etc. if needed by broker
        ...
        try:
            ...
            if order:
                ...
                self.order_retry_counts[order.ref] = 0
                ...
                if trade_cycle_id:
                    ...
                    # Check if buy_bracket/sell_bracket was used indirectly
                ...
                submit_log_data = {
                    # FR-ID-004
                    "order_ref": order.ref if not isinstance(order, list) else [o.ref for o in order if o],
                ...
                self.log_event(event_type=ORDER_SUBMITTED,
                               data_feed=data, **submit_log_data)
                # --------------------------
    ...
    def next(self):
        ...
        self.log_event(MARKET_STATE_ASSESSED,
                       data_feed=self.datas[0], **market_log_data)
        # -----------------------------------
        ...
        if position_size != 0:
            ...
            if position_size > 0:  # Long position
                ...
                if self.data_low[0] <= stop_loss_price:
                    ...
                    self.log_event(SL_TP_TRIGGERED,
                                   data_feed=self.datas[0], **sl_tp_log_data)
        ...
        # (If there is no open position, check for entry signals.)
            ...
            # Example: RSI above 50 indicates potential strength
            long_condition_rsi = self.rsi[0] > 50
            # MACD crossover
            long_condition_macd = self.macd.macd[0] > self.macd.signal[
                0] and self.macd.macd[-1] <= self.macd.signal[-1]
            # Price above middle band
            long_condition_bbands = self.data_close[0] > self.bbands.mid[0]
            ...
            # Example: RSI below 50 indicates potential weakness
            short_condition_rsi = self.rsi[0] < 50
            # MACD crossover
            short_condition_macd = self.macd.macd[0] < self.macd.signal[
                0] and self.macd.macd[-1] >= self.macd.signal[-1]
            # Price below middle band
            short_condition_bbands = self.data_close[0] < self.bbands.mid[0]
            ...
            if is_long_signal:
                ...
                self.log_event(SIGNAL_TRIGGERED,
                               data_feed=self.datas[0], **signal_log_data)
                # ----------------------------

                # Use closing price for calculation/entry
                ...
                size = self._calculate_trade_size(
                    self.data_close[0], entry_price, stop_loss_price, self.p.risk_per_trade_percent, data=self.datas[0])
                # ORDER_CALCULATION log is done inside _calculate_trade_size

                if size > 0:
                    # Place the order
                    self._place_order(
                        data=self.datas[0], order_type='buy', size=size, exectype=bt.Order.Market)
                ...
                if size > 0:
                    self._place_order(
                        data=self.datas[0], order_type='sell', size=size, exectype=bt.Order.Market)
...
def load_data_to_cerebro(cerebro, data_files, column_mapping, openinterest_col, fromdate, todate):
    ...
    for i, file_path in enumerate(data_files):
        ...
        try:
            ...
            # 重新引发异常，以便上层调用者知道加载失败。 (Re-raise the exception so the caller knows loading failed.)
            raise
...
def analyze_optimization_results(results):
    ...
    best_result = None
    # 初始化为负无穷大，确保任何正收益都会被选中 (Initialize to negative infinity to ensure any positive return is selected)
    ...
    for i, run in enumerate(results):
        ...
        # 第一个元素是策略实例 (The first element is the strategy instance)
        strategy = run[0]
        # 获取当前运行的参数 (Get parameters for the current run)
        ...
        for aname, analyzer in analyzers:
            ...
            if isinstance(analyzer, bt.analyzers.SharpeRatio):
                ...
                # 存储整个交易分析结果 (Store the entire trade analysis result)
        ...
        # 将百分比转换为小数加入评分 (Convert percentage to decimal and add to score)
        ...
        if drawdown is not None and drawdown > 0:
            # 示例：回撤惩罚权重为0.5 (Example: Drawdown penalty weight 0.5)
            score -= (drawdown / 100) * 0.5

        # 4. 系统质量：SQN (加分项，越高越好)
        # 4. System Quality: SQN (Bonus points, higher is better)
        if sqn is not None:
            # 示例：SQN 评分权重为 0.1 (Example: SQN score weight 0.1)
            score += sqn / 10
        ...
        result_summary = {
            ...
            'sqn': sqn,
            # 包含详细交易统计 (Includes detailed trade statistics)
            'trade_analysis': trade_analysis,
            'score': score  # 添加综合评分 (Add composite score)
        }
        all_results.append(result_summary)
        # 也添加到带评分列表 (Add to scored list as well)
        all_scored_results.append(result_summary)
...
def print_optimization_summary(best_result, all_scored_results, ema_medium_range, ema_long_range, bbands_period_range, bbands_dev_range, total_time, actual_combinations, avg_time):
    ...
    separator = "-" * 165
    print(separator)
    print(header.format("Rank", "Parameters", "Final Value", "PnL %", "Score",
          "Sharpe Ratio", "Max DD %", "SQN", "Total Tr", "Win Tr", "Loss Tr", "Avg PNL/Tr"))
    print(separator)
    ...
    def format_params(params):
        ...
        opt_keys = ['ema_medium_period', 'ema_long_period',
                    'bbands_period', 'bbands_devfactor']
    ...
    if best_result:
        ...
        if best_result.get('trade_analysis'):
            ...
            if ta and 'total' in ta and 'closed' in ta['total']:
                print(f"    总交易次数: {ta['total']['closed']}")  # Total Trades
            if ta and 'won' in ta and 'total' in ta['won']:
                print(f"    盈利次数: {ta['won']['total']}")  # Winning Trades
            if ta and 'lost' in ta and 'total' in ta['lost']:
                print(f"    亏损次数: {ta['lost']['total']}")  # Losing Trades
            if ta and 'long' in ta and 'total' in ta['long']:
                print(f"    多头交易: {ta['long']['total']}")  # Long Trades
            if ta and 'short' in ta and 'total' in ta['short']:
                print(f"    空头交易: {ta['short']['total']}")  # Short Trades
            if ta and 'pnl' in ta and 'net' in ta['pnl']:
                # Total Net PnL
                print(f"    总净利润: {ta['pnl']['net']['total']:.2f}")
                # Average Net PnL per Trade
                print(f"    平均净利润/笔: {ta['pnl']['net']['average']:.2f}")
            if ta and 'won' in ta and 'pnl' in ta['won'] and 'average' in ta['won']['pnl']:
                # Average Winning Trade
                print(f"    平均盈利/笔: {ta['won']['pnl']['average']:.2f}")
            if ta and 'lost' in ta and 'pnl' in ta['lost'] and 'average' in ta['lost']['pnl']:
                # Average Losing Trade
                print(f"    平均亏损/笔: {ta['lost']['pnl']['average']:.2f}")
...
def print_backtest_summary(initial_cash, final_value, strat):
    ...
    print(separator)
    print(header.format("Final Value", "PnL %", "Score*", "Sharpe Ratio",
          "Max DD %", "SQN", "Total Tr", "Win Tr", "Loss Tr", "Avg PNL/Tr"))
    print(separator)
    ...
    if trade_analysis:
        ...
        if 'pnl' in trade_analysis and 'net' in trade_analysis['pnl'] and 'average' in trade_analysis['pnl']['net']:
            avg_pnl_per_trade = f"{trade_analysis['pnl']['net']['average']:.2f}"
    ...
    if trade_analysis:
        ...
        if 'long' in trade_analysis and 'total' in trade_analysis['long']:
            print(f"    多头交易: {trade_analysis['long']['total']}")
        if 'short' in trade_analysis and 'total' in trade_analysis['short']:
            print(f"    空头交易: {trade_analysis['short']['total']}")
        if 'pnl' in trade_analysis and 'net' in trade_analysis['pnl']:
            print(f"    总净利润: {trade_analysis['pnl']['net']['total']:.2f}")
            print(
                f"    平均净利润/笔: {trade_analysis['pnl']['net']['average']:.2f}")
        if 'won' in trade_analysis and 'pnl' in trade_analysis['won'] and 'average' in trade_analysis['won']['pnl']:
            print(f"    平均盈利/笔: {trade_analysis['won']['pnl']['average']:.2f}")
        if 'lost' in trade_analysis and 'pnl' in trade_analysis['lost'] and 'average' in trade_analysis['lost']['pnl']:
            print(
                f"    平均亏损/笔: {trade_analysis['lost']['pnl']['average']:.2f}")
...
def get_backtest_config():
    ...
    return {
        ...
        # -1表示不使用持仓量列 (means do not use openinterest column)
        "openinterest_col": -1,
        ...
        "optimization": {
            ...
            # 优化参数范围 (Optimization parameter range)
            "ema_medium_period": range(20, 31, 2),
        ...
        "plot": {  # 绘图设置 (Plotting settings)
            ...
            # Plot style ('bar', 'line', 'candlestick')
            "style": 'candlestick',
            "volume": True,  # Show volume subplot
            # Indicators on main chart
            "indicators_on_main": ['ema_short', 'ema_medium', 'ema_long', 'bbands'],
            # Indicators on separate subplots
            "indicators_off_main": ['rsi', 'macd']
        }
    }
...
def add_strategy_components(cerebro, config):
    ...
    if config['optimization']['enabled']:
        ...
        strategy_opt_params = {k: v for k,
                               v in opt_params.items() if k != 'enabled'}
    ...
    cerebro.addanalyzer(bt.analyzers.SharpeRatio, _name='sharpe_ratio', timeframe=bt.TimeFrame.Days, compression=1,
                        factor=252, annualize=True)  # 假设日数据，年化因子252 (Assuming daily data, annualization factor 252)
    ...
    if config['optimization']['enabled']:
        ...
        if config['plot']['enabled']:
            ...
            try:
                ...
                indicators_on_main = config['plot'].get(
                    'indicators_on_main', [])
                indicators_off_main = config['plot'].get(
                    'indicators_off_main', [])
                ...
                # 使用解包传递参数 (Use unpacking to pass arguments)
