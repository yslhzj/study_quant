...
def setup_logging(log_dir="logs", log_level=logging.INFO):
    ...
    log_file = os.path.join(
        log_dir, f"bt_strategy_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.jsonl")
        ...
        for i, d in enumerate(self.datas):
            ...
            if (sma20 > sma60 > sma90) and pos <= 0 and self.p.longshort in ['long', 'both']:
                ...
                size = self.get_trade_size(
                    d, self.broker.get_cash(), d.close[0])
                ...
                trade_cycle_id = self.generate_trade_cycle_id()
                # 确定卖出规模
                size = abs(pos) if pos != 0 else self.get_trade_size(
                    d, self.broker.get_cash(), d.close[0])
