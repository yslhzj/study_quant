...
# 参数优化相关代码
class OptStrategy(AShareETFStrategy):
    """
    参数优化策略类，继承自AShareETFStrategy
    用于在回测过程中评估不同参数组合的表现
    """
    ...
    def stop(self):
        ...
        # 获取夏普比率
        sharpe = self.analyzers.sharpe_ratio.get_analysis().get('sharperatio', 0)

        # 计算优化目标函数
        # 这里我们使用夏普比率作为主要优化目标，同时考虑最大回撤
        # 可以根据需要调整优化目标
        optimization_target = sharpe * (1 - drawdown/100)

        # 打印当前参数组合的结果
        print(f"参数组合: {self.p._getkwargs()}")
        print(f"最终账户价值: {final_value:.2f}")
        print(f"总收益率: {total_return*100:.2f}%")
        print(f"最大回撤: {drawdown:.2f}%")
        print(f"夏普比率: {sharpe:.2f}")
        print(f"优化目标值: {optimization_target:.4f}")
        print("-" * 50)
...
def run_optimization():
    ...
    # 设置优化参数范围
    # 趋势策略参数
    ema_medium_periods = range(20, 100, 20)  # 20, 40, 60, 80
    ema_long_periods = range(60, 180, 30)    # 60, 90, 120, 150
    adx_periods = range(10, 30, 5)           # 10, 15, 20, 25
    atr_periods = range(10, 30, 5)           # 10, 15, 20, 25
    trend_stop_loss_mults = [2.0, 2.5, 3.0]
    trend_take_profit_ratios = [1.5, 2.0, 2.5]

    # 震荡策略参数
    bbands_periods = range(10, 30, 5)        # 10, 15, 20, 25
    bbands_devfactors = [1.5, 2.0, 2.5]
    rsi_periods = range(10, 30, 5)           # 10, 15, 20, 25
    ...
    cerebro.optstrategy(
        OptStrategy,
        etf_type=['trend', 'range'],  # 测试两种策略类型
        ema_medium_period=ema_medium_periods,
        ema_long_period=ema_long_periods,
        adx_period=adx_periods,
        atr_period=atr_periods,
        trend_stop_loss_atr_mult=trend_stop_loss_mults,
        trend_take_profit_rratio=trend_take_profit_ratios,
        bbands_period=bbands_periods,
        bbands_devfactor=bbands_devfactors,
    ...
    # 运行优化
    print("开始参数优化...")
    opt_results = cerebro.run(maxcpus=1)  # 使用单核运行，避免内存问题
    ...
    for result in opt_results:
        for strat in result:
            if strat.analyzers.sharpe_ratio.get_analysis().get('sharperatio', 0) > best_value:
                best_value = strat.analyzers.sharpe_ratio.get_analysis().get('sharperatio', 0)
                best_result = strat

    if best_result:
        print("\n最优参数组合:")
        print(best_result.p._getkwargs())
