...
def optimize_strategy():
    ...
    for file_path in data_files:
        try:
            ...
            if 'datetime' in dataframe.columns:
                ...
                print(f"错误: 在 {file_path} 中找不到datetime列")
                continue
    ...
    cerebro.optstrategy(
        ...
        ema_medium_period=range(20, 61, 20),  # [20, 40, 60]
    ...
    for i, result in enumerate(results):
        ...
        dd = analyzers.drawdown.get_analysis().max.drawdown
        ret = analyzers.returns.get_analysis()['rtot']

        # 打印当前参数组合结果
        print(f"组合 {i+1}:")
        print(f"参数:")
        print(f"  策略类型: {params.etf_type}")
        print(f"  中期均线周期: {params.ema_medium_period}")
        print(f"  长期均线周期: {params.ema_long_period}")
        print(f"  ATR止损倍数: {params.trend_stop_loss_atr_mult}")
        print(f"  布林带周期: {params.bbands_period}")
        print(f"  RSI超卖阈值: {params.rsi_oversold}")
        print(f"  趋势最大风险: {params.max_risk_per_trade_trend}")
        print(f"  最大仓位比例: {params.max_position_per_etf_percent}")
        print(f"绩效指标:")
        ...
        if sharpe > best_sharpe:
            best_sharpe = sharpe
            best_params = params
            best_metrics = (sharpe, dd, ret)

    # 打印最佳结果
    print("\n===== 最佳参数组合 =====")
    print(f"策略类型: {best_params.etf_type}")
    print(f"中期均线周期: {best_params.ema_medium_period}")
    print(f"长期均线周期: {best_params.ema_long_period}")
    print(f"ATR止损倍数: {best_params.trend_stop_loss_atr_mult}")
    print(f"布林带周期: {best_params.bbands_period}")
    print(f"RSI超卖阈值: {best_params.rsi_oversold}")
    print(f"趋势最大风险: {best_params.max_risk_per_trade_trend}")
    print(f"最大仓位比例: {best_params.max_position_per_etf_percent}")
    print("\n最佳绩效:")
    print(f"夏普比率: {best_metrics[0]:.2f}")
    print(f"最大回撤: {best_metrics[1]:.2f}%")
    print(f"总收益率: {best_metrics[2]:.2f}%")
