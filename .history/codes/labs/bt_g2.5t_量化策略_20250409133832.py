...
class AShareETFStrategy(bt.Strategy):
    ...
    def log(self, txt, dt=None, data=None):
        ...
        return
        # 函数体返回None，实际日志记录功能被注释掉了，目前不执行任何操作。 (这行代码啥也不干，直接结束函数，所以现在log函数实际上没用。)
    ...
    def __init__(self):
        ...
        for i, d in enumerate(self.datas):
            ...
            print(self.adxs)
            # 打印self.adxs变量，用于查看其中存储的数据。(打印self.adxs这个列表，看看里面装了啥。)
            # 为当前数据源计算ADX指标，并添加到self.adxs列表。 (计算当前ETF的ADX指标，然后放到列表中。)
    ...
    def notify_order(self, order):
        ...
        if order.status in [order.Completed]:
            ...
            if order.isbuy():
                ...
                self.log(f'BUY EXECUTED, Price: {order.executed.price:.2f}, Cost: {order.executed.value:.2f}, Comm {order.executed.comm}', data=order_data)
                # 记录买入成交的日志，包括成交价格、总成本和佣金。 (记录买入成功的日志，包括成交价格、花了多少钱、手续费多少。)
                ...
                self.log(f'SELL EXECUTED, Price: {order.executed.price:.2f}, Cost: {order.executed.value:.2f}, Comm {order.executed.comm}', data=order_data)
                # 记录卖出成交的日志，包括成交价格、总价值和佣金。 (记录卖出成功的日志，包括成交价格、卖了多少钱、手续费多少。)
            self.bar_executed = len(order_data)
            # 记录订单执行时的数据索引（K线索引）。 (记录这笔交易是在第几根K线成交的。)

        elif order.status in [order.Canceled, order.Margin, order.Rejected]:
            # 检查订单状态是否为已取消、保证金不足或被拒绝。 (判断订单是不是被取消了、钱不够买、或者被券商拒绝了。)
            self.log(f'Order Canceled/Margin/Rejected: Status {order.getstatusname()}', data=order_data)
            # 记录订单取消、保证金不足或被拒绝的日志，包含订单状态名称。 (记录订单失败的日志，说明订单被取消、钱不够或者被拒绝了，并说明具体原因。)

        if order_data in self.orders:
            # 确保订单数据源在self.orders字典中存在。 (确保这个订单对应的ETF是在我们监控的列表里的。)
            self.orders[order_data] = None
            # 重置当前数据源的订单状态为None，表示没有挂单。 (把当前ETF的订单状态清空，表示现在没有挂单了。)
    ...
    def notify_trade(self, trade):
        ...
        self.log(f'OPERATION PROFIT, GROSS {trade.pnl:.2f}, NET {trade.pnlcomm:.2f}', data=trade.data)
        # 记录交易的利润日志，包括毛利润（pnl）和净利润（pnlcomm，扣除佣金）。 (记录这次交易赚了多少钱，包括毛利润和扣掉手续费的净利润。)
