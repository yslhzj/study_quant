...
class AShareETFStrategy(bt.Strategy):
    ...
    def notify_cashvalue(self, cash, value):
        ...
        if drawdown > self.params.drawdown_level2_threshold:
            ...
            if not self.halt_trading:
                ...
                self.log(f'!!! RED ALERT: Drawdown {drawdown*100:.2f}% > {self.params.drawdown_level2_threshold*100:.0f}%. HALTING TRADING !!!', data=None) # 全局信息，不关联特定data
            ...
            if not self.drawdown_level1_triggered:
                ...
                self.log(f'-- YELLOW ALERT: Drawdown {drawdown*100:.2f}% > {self.params.drawdown_level1_threshold*100:.0f}%. Reducing risk.--', data=None) # 全局信息
    ...
    def _calculate_trade_size(self, data_close_price, entry_price, stop_loss_price, risk_per_trade_percent, data=None):
        ...
        if stop_loss_price >= entry_price:
            ...
            self.log(f"Stop loss price {stop_loss_price:.2f} is not below entry price {entry_price:.2f}. Cannot calculate size.", data=data)
        ...
        if risk_per_share <= 0:
            ...
            self.log(f"Calculated risk per share is zero or negative ({risk_per_share:.2f}). Cannot calculate size.", data=data)
        ...
        for d, pos in self.broker.positions.items():
            ...
            if pos.size != 0 and d in self.stop_loss_prices and self.stop_loss_prices[d] is not None:
                 ...
                 # 获取止损价。 (获取这只ETF的止损价。)
                 risk_per_share_existing = current_price - sl_price if pos.size > 0 else sl_price - current_price # 做空情况暂不考虑
                 # 计算现有持仓的每股风险。 (计算如果现在跌到止损价，每股会亏多少钱。)
                 if risk_per_share_existing > 0:
                     # 如果每股风险大于0。 (如果算出来的风险是正数。)
                     total_current_risk += abs(pos.size) * risk_per_share_existing
                     # 累加当前持仓的总风险。 (把这个ETF的总风险加到账户总风险里。)

        potential_new_risk = (equity * effective_risk_percent) # 简化计算，以目标风险金额作为潜在新风险
        ...
        if (total_current_risk + potential_new_risk) / equity > self.params.max_total_account_risk_percent:
            ...
            self.log(f"Trade skipped due to exceeding max total account risk limit. Current risk: {(total_current_risk/equity)*100:.2f}%, New trade risk: {effective_risk_percent*100:.2f}%, Limit: {self.params.max_total_account_risk_percent*100:.2f}%", data=data)
        ...
        if size <= 0:
            ...
            self.log(f"Calculated size is zero or negative ({size}). Cannot place order.", data=data)
        ...
        if current_price_for_calc <= 0:
             ...
             self.log(f"Invalid price ({current_price_for_calc:.2f}) for size calculation.", data=data)
        ...
        if potential_trade_value > max_pos_value:
            ...
            self.log(f"Size reduced from {size} to {new_size} due to max position limit ({self.params.max_position_per_etf_percent*100:.0f}% of equity).", data=data)
        ...
        if potential_trade_value > cash:
            ...
            if new_size < size: # 只有当现金限制导致规模减小时才记录
                self.log(f"Size reduced from {size} to {new_size} due to cash limit (Available: {cash:.2f}, Required: {potential_trade_value:.2f}).", data=data)
        ...
        if size <= 0:
             ...
             self.log(f"Final calculated size is zero or negative ({size}). Cannot place order.", data=data)
        ...
        for i, d in enumerate(self.datas):
            ...
            if position.size != 0:
                ...
                if stop_loss is not None and current_price <= stop_loss:
                    ...
                    self.log(f'STOP LOSS HIT: Closing {data_name} at {current_price:.2f} (Stop @ {stop_loss:.2f})', data=d)
                ...
                # Check take-profit
                    ...
                    self.log(f'TAKE PROFIT HIT: Closing {data_name} at {current_price:.2f} (Profit Target @ {take_profit:.2f})', data=d)
            ...
            # If no position, check for entry conditions
                            ...
                            if stop_loss_price_calc < entry_price_calc:
                                ...
                                if risk_per_share > 0:
                                    ...
                                    # 根据盈亏比计算止盈价，入场价加上风险金额的倍数。 (根据设定的盈亏比（比如2倍），算出止盈价应该设在假定入场价上方多少。)
                                     ...
                                     # 取消入场信号。 (那就取消买入信号。)
                                     self.log(f"Trend signal {data_name} skipped: Risk per share is non-positive ({risk_per_share:.2f}). SL: {stop_loss_price_calc:.2f}, Entry: {entry_price_calc:.2f}", data=d)
                                     # 记录日志。 (记录日记：趋势信号跳过，因为每股风险有问题。)

                            else:
                                entry_signal = False # 止损价不低于入场价，取消信号
                                # 如果止损价无效，则取消入场信号。 (如果止损价有问题，那就取消买入信号。)
                                self.log(f"Trend signal {data_name} skipped: Stop loss {stop_loss_price_calc:.2f} not below entry {entry_price_calc:.2f}", data=d)
                    ...
                    try:
                        ...
                        if is_range_buy:
                            ...
                            if stop_loss_price_calc >= entry_price_calc:
                                ...
                                entry_signal = False # 止损价不低于入场价，取消信号
                                # 如果止损价无效，则取消入场信号。 (如果止损价有问题，那就取消买入信号。)
                                self.log(f"Range signal {data_name} skipped: Stop loss {stop_loss_price_calc:.2f} not below entry {entry_price_calc:.2f}", data=d)
                ...
                if entry_signal and stop_loss_price_calc is not None and entry_price_calc > stop_loss_price_calc:
                    ...
                    size = self._calculate_trade_size(self.closes[i][0], entry_price_calc, stop_loss_price_calc, risk_per_trade_percent, data=d)
                    # 调用_calculate_trade_size方法计算仓位大小。 (让小工具帮忙算算买多少股。)

                    if size > 0:
                        # 检查计算出的仓位大小是否大于0。 (如果算了一圈下来，确实还能买 > 0 股这只ETF。)
                        self.log(f'BUY SIGNAL: {data_name}, Size: {size}, Entry: ~{entry_price_calc:.2f}, SL: {stop_loss_price_calc:.2f}, TP: {take_profit_price_calc if take_profit_price_calc else "N/A"}, Type: {potential_position_type}', data=d)
                        ...
                        if order_buy:
                             ...
                             self.stop_loss_prices[d] = stop_loss_price_calc # 记录计划的止损价
                             # 记录这笔交易计划的止损价。 (把算好的止损价记下来。)
                             self.take_profit_prices[d] = take_profit_price_calc # 记录计划的止盈价
                             # 记录这笔交易计划的止盈价。 (把算好的止盈价记下来。)
                             self.position_types[d] = potential_position_type # 记录持仓类型
                             # 记录这次是按哪种策略买的。 (把这次是按趋势还是震荡买的记下来。)
                        else:
                             # 如果买入订单创建失败。 (如果买入指令没发出去。)
                             self.log(f'Failed to create buy order for {data_name}', data=d)
                             # 记录日志。 (记录日志：买入失败。)


if __name__ == '__main__':
    # Python主程序入口，当直接运行此脚本时，以下代码会被执行。 (这是程序的入口，只有直接运行这个文件的时候，下面的代码才会跑。)
    cerebro = bt.Cerebro(stdstats=False, optreturn=False) # optreturn=False保留策略实例以便访问分析器
    ...
    cerebro.addanalyzer(bt.analyzers.SharpeRatio, _name='sharpe_ratio', timeframe=bt.TimeFrame.Days, riskfreerate=0.0) # 假设无风险利率为0
        ...
        for strat_list in results: # optreturn=False时，results是列表的列表
            ...
            try:
                ...
                if 'rtot' not in returns_analysis or 'max' not in drawdown_analysis or 'drawdown' not in drawdown_analysis.max:
                     ...
                     print(f"警告: 参数组 {params.ema_medium_period}, {params.ema_long_period}, {params.bbands_period}, {params.bbands_devfactor} 的分析结果不完整，跳过。")
                ...
                max_drawdown = drawdown_analysis.max.get('drawdown', 0.0) / 100.0 # 最大回撤（小数）
                # 从最大回撤分析器中获取最大回撤（转换为小数形式）。 (从最大回撤分析器里拿到最大回撤，把它变成小数形式，比如10%变成0.10。)

                # 计算得分 Score = (Return * 0.6) + ((1 - Drawdown) * 0.4)
                # Calculate score
                score = (total_return * 0.6) + ((1 - max_drawdown) * 0.4) if max_drawdown < 1 else total_return * 0.6 # 防止回撤100%导致 (1-dd)为0
                ...
                # 打印当前参数组合及其回测结果，包括新的得分。 (打印当前参数组合的回测结果，包括参数值、收益率%、最大回撤%、得分。)
                 ...
                 print(f"错误: 处理参数组 {params.ema_medium_period if params else 'N/A'} 时出错: {e}")
                 # 打印错误信息。 (打印错误信息。)
