                ...
                try:
                    ...
                    if is_breakout or is_pullback:
                        ...
                        if math.isnan(atr_val) or atr_val <= 1e-9:
                             self.log(f"{d_name} ATR值无效 ({atr_val:.4f})，无法计算趋势止损。跳过。", data=d_obj)
                             # (Invalid ATR value ({atr_val:.4f}) for {d_name}, cannot calculate trend stop loss. Skipping.)
                             entry_signal = False # 取消信号
                             # (Cancel signal)
                        else:
                            stop_loss_price_calc = entry_price_calc - self.p.trend_stop_loss_atr_mult * atr_val
                            # 计算止损价格。
                            # (Calculate stop loss price.)

                            if stop_loss_price_calc >= entry_price_calc:
                                self.log(f"{d_name} 趋势止损价 {stop_loss_price_calc:.2f} 不低于入场价 {entry_price_calc:.2f}。跳过。", data=d_obj)
                                ...
                                if risk_per_share <= 1e-9:
                                    self.log(f"{d_name} 趋势交易每股风险过小 ({risk_per_share:.2f})。跳过。", data=d_obj)
                                    # ({d_name} trend trade risk per share too small ({risk_per_share:.2f}). Skipping.)
                                    entry_signal = False
                                    # 取消入场信号。
                                    # (那就不买了。)
                                else:
                                    take_profit_price_calc = entry_price_calc + self.p.trend_take_profit_rratio * risk_per_share
                                    # 计算止盈价格。
            ...
            # (--- Range Trading Signal Check ---)
                ...
                try:
                    ...
                    if is_range_buy:
                        ...
                        if stop_loss_price_calc >= entry_price_calc:
                            self.log(f"{d_name} 区间止损价 {stop_loss_price_calc:.2f} 不低于入场价 {entry_price_calc:.2f}。跳过。", data=d_obj)
                            ...
                            # (Calculate risk per share.)
                            if risk_per_share <= 1e-9:
                                self.log(f"{d_name} 区间交易每股风险过小 ({risk_per_share:.2f})。跳过。", data=d_obj)
            ...
            if entry_signal and entry_price_calc is not None and \
               ...
               amount_to_risk is not None and entry_price_calc > stop_loss_price_calc :
                ...
                if risk_per_share <= 1e-9 or amount_to_risk <= 1e-9:
                    self.log(f"信号产生但风险计算无效 for {d_name} (Risk/Share: {risk_per_share:.4f}, AmountToRisk: {amount_to_risk:.2f})。跳过。", data=d_obj)
                ...
                self.log(f"为 {d_name} 准备交易信息: 入场={entry_price_calc:.2f}, 止损={stop_loss_price_calc:.2f}, 风险/股={risk_per_share:.4f}, 允许风险额={amount_to_risk:.2f}", data=d_obj)
                # (记个日志说交易计划准备好了。)
                ...
                # (If take profit is valid, use it)
                    ...
                    self.log(f"警告 for {d_name}: 趋势交易止盈价 {take_profit_price_calc:.2f} 无效 (不高于入场价 {main_order_limit_price:.2f})。括号单将无止盈限价单。", data=d_obj)
                ...
                self.log(f"发出买入括号单信号 for {d_name}, 参考入场: {main_order_limit_price:.2f}, 止损: {stop_loss_price_calc:.2f}, 止盈: {tp_price_for_bracket if tp_price_for_bracket else 'N/A'}", data=d_obj)
                ...
                if bracket_orders_list and bracket_orders_list[0]:
                    ...
                    self.position_types[d_name] = potential_position_type # 记录持仓类型
                    # self.position_types[d_name] = potential_position_type # Record position type
                    self.log(f"成功为 {d_name} 创建 buy_bracket 请求。主订单 ref: {bracket_orders_list[0].ref if bracket_orders_list[0] else 'N/A'}", data=d_obj)
                    ...
                    self.log(f"为 {d_name} 创建 buy_bracket 失败 (可能Sizer返回0或错误)", data=d_obj)
    ...
    for file_path in data_files:
        try:
            ...
            if 'datetime' in dataframe.columns:
                ...
                print(f"错误: 在 {file_path} 中找不到映射后的 'datetime' 列。请确保Excel文件中有正确的日期列，或正确修改脚本中的column_mapping。")
            ...
            if isinstance(openinterest_col, str) and openinterest_col in dataframe.columns:
                ...
                print(f"警告: 指定的 openinterest 列名 '{openinterest_col}' 在 {file_path} 中不存在。将忽略持仓量。")
                ...
                print(f"警告: 为 openinterest 提供了整数索引 {openinterest_col}。如果意图是列名，请使用字符串。否则，行为可能未定义。")
...
def analyze_optimization_results(results):
    ...
    processed_results = []
    print("\n--- 开始提取分析结果 ---")
    # 注意：optreturn=False 时，results 是列表的列表
    # (Note: When optreturn=False, results is a list of lists)
    run_count = 0
    successful_runs = 0
    for strat_list in results:
        run_count += 1
        if not strat_list:
            # print(f"警告: 第 {run_count} 次运行结果为空列表，跳过。")
            continue # 跳过空列表
        ...
        # 构造参数字符串用于日志/调试
        # (Construct parameter string for logging/debugging)
        params_str_parts = []
        # (Initialize list for parameter string parts)
        optimized_param_names = [ # 应该与 optstrategy 中的参数一致
        # (Should match parameters in optstrategy)
        ...
        for p_name in optimized_param_names:
             if hasattr(params, p_name):
                ...
                # (If attribute exists, append key-value pair)
             else:
                params_str_parts.append(f"{p_name}=MISSING")
                # (If attribute is missing, mark as missing)
        params_str = ", ".join(params_str_parts)
        # (Join parts into a comma-separated string)
        ...
        try:
            ...
            if not valid_analysis:
                 # print(f"警告: 参数组 [{params_str}] 的分析结果不完整，跳过。")
                 continue # 跳过无效分析结果的运行
                 # (Skip run with incomplete analysis results)


            sharpe = sharpe_analysis.get('sharperatio') # Sharpe Ratio 可能为 None
            # sharpe = sharpe_analysis.get('sharperatio', 0.0) # Sharpe Ratio might be None
            if sharpe is None: sharpe = 0.0 # 再次确认
            # if sharpe is None: sharpe = 0.0 # Double check
            total_return = returns_analysis.get('rtot', 0.0) # 总收益率（小数）
            # total_return = returns_analysis.get('rtot', 0.0) # Total return (decimal)
            max_drawdown = drawdown_analysis.get('max', {}).get('drawdown', 0.0) / 100.0 # 最大回撤（转换为小数）
            # max_drawdown = drawdown_analysis.max.get('drawdown', 0.0) / 100.0 # Max drawdown (convert to decimal)
            ...
            # 使用上面定义的 optimized_param_names
            # (Use optimized_param_names defined above)
            for p_name in optimized_param_names:
                if hasattr(params, p_name):
            ...
            processed_results.append({
                ...
                'params_dict': current_params_dict, # 存储参数字典
                # 'params_dict': current_params_dict, # Store parameter dictionary
                'sharpe': sharpe,
                'return': total_return,
                'drawdown': max_drawdown
            })
            successful_runs += 1 # 成功处理的运行次数加1
            # successful_runs += 1 # Increment count of successfully processed runs

        except AttributeError as e:
             # print(f"错误: 处理参数组 [{params_str}] 时遇到属性错误 (可能是分析器结果缺失): {e}")
             # (Error: Attribute error encountered while processing parameter set [{params_str}] (possibly missing analyzer results): {e})
             pass # 捕获但不中断，继续处理其他结果
             # (Catch but don't break, continue processing other results)
        except Exception as e:
            # print(f"错误: 处理参数组 [{params_str}] 时出错: {e}")
            # (Error: Error encountered while processing parameter set [{params_str}]: {e})
            import traceback
            # traceback.print_exc()
            pass # 捕获但不中断
            # (Catch but don't break)

    print(f"--- 完成提取分析。总运行次数: {run_count}, 成功提取结果: {successful_runs} ---")
    # (--- Finished extracting analysis. Total runs: {run_count}, Successfully extracted results: {successful_runs} ---)
    ...
    # 从所有处理结果中提取每个策略的夏普比率值
    # (把每个回测结果的夏普比率都拿出来放到一个列表里)

    # 提取所有结果中的收益率
    all_returns = [r['return'] for r in processed_results]
    # 从所有处理结果中提取每个策略的总收益率值
    # (把每个回测结果的收益率都拿出来放到一个列表里)

    # 提取所有结果中的最大回撤
    all_drawdowns = [r['drawdown'] for r in processed_results]
    # 从所有处理结果中提取每个策略的最大回撤值
    # (把每个回测结果的最大回撤都拿出来放到一个列表里)

    # 计算夏普比率的最小值
    min_sharpe = min(all_sharpes) if all_sharpes else 0.0
    # 如果夏普比率列表不为空，则取其最小值，否则默认为0
    # (如果有夏普比率数据，就找出最小的那个，没有就用0)

    # 计算夏普比率的最大值
    max_sharpe = max(all_sharpes) if all_sharpes else 0.0
    # 如果夏普比率列表不为空，则取其最大值，否则默认为0
    # (如果有夏普比率数据，就找出最大的那个，没有就用0)

    # 计算收益率的最小值
    min_return = min(all_returns) if all_returns else 0.0
    # 如果收益率列表不为空，则取其最小值，否则默认为0
    # (如果有收益率数据，就找出最小的那个，没有就用0)

    # 计算收益率的最大值
    max_return = max(all_returns) if all_returns else 0.0
    # 如果收益率列表不为空，则取其最大值，否则默认为0
    # (如果有收益率数据，就找出最大的那个，没有就用0)

    # 计算回撤的最小值
    min_drawdown = min(all_drawdowns) if all_drawdowns else 0.0
    # 如果回撤列表不为空，则取其最小值，否则默认为0
    # (如果有回撤数据，就找出最小的那个，没有就用0)

    # 计算回撤的最大值
    max_drawdown_val = max(all_drawdowns) if all_drawdowns else 0.0
    # 如果回撤列表不为空，则取其最大值，否则默认为0
    # (如果有回撤数据，就找出最大的那个，没有就用0)

    # 初始化最佳得分和结果
    best_score = float('-inf')
    # 设置初始最佳得分为负无穷大，确保任何得分都能更新它
    # (先把最佳得分设为负无穷大，这样任何策略的得分都会比它大)

    # 初始化最佳结果数据为空
    best_result_data = None
    # 用于存储得分最高的策略结果
    # (先把最佳结果设为空，后面会用得分最高的策略来填充它)

    # 初始化得分结果列表
    scored_results = []
    # 用于存储所有带得分的策略结果
    # (创建一个空列表，用来存放所有计算过得分的策略结果)

    # 打印归一化得分计算开始信息
    print("\n--- 开始计算归一化得分 ---")
    # 输出提示信息，表明开始计算归一化得分
    # (告诉用户现在开始计算每个策略的标准化得分)
    ...
    # 输出各指标的取值范围，保留4位小数
    # (显示夏普比率、收益率和回撤的最小值和最大值，方便了解数据分布)
    ...
    for result_data in processed_results:
        ...
        # 计算所有夏普比率的取值范围
        # (计算所有策略中夏普比率的最大值和最小值之差)

        # 计算收益率的范围
        return_range = max_return - min_return
        # 计算所有收益率的取值范围
        # (计算所有策略中收益率的最大值和最小值之差)
        ...
        # 将夏普比率归一化到0-1范围，如果范围过小则设为0
        # (把夏普比率转换成0到1之间的值，方便比较不同指标)

        # 归一化收益率
        return_norm = (ret - min_return) / return_range if return_range > 1e-9 else 0.0
        # 将收益率归一化到0-1范围，如果范围过小则设为0
        # (把收益率转换成0到1之间的值，方便比较不同指标)

        # 归一化回撤
        drawdown_norm = (dd - min_drawdown) / drawdown_range if drawdown_range > 1e-9 else 0.0
        # 将回撤归一化到0-1范围，如果范围过小则设为0
        # (把回撤转换成0到1之间的值，方便比较不同指标)

        # 计算综合得分
        score = 0.6 * sharpe_norm + 0.1 * return_norm - 0.3 * drawdown_norm
        # 使用加权方式计算综合得分，夏普比率权重0.6，收益率权重0.1，回撤权重-0.3
        # (计算策略的总得分：夏普比率占60%，收益率占10%，回撤扣30%分)
    ...
    # 输出提示信息，表明得分计算完成，并显示计算的结果数量
    # (告诉用户已经完成了所有策略的得分计算，并显示总共计算了多少组)

    # 返回最佳结果和所有得分结果
    return best_result_data, scored_results
    # 函数返回最佳结果数据和所有带得分的结果列表
    # (把最佳策略和所有计算过得分的策略结果都返回出去)
...
if __name__ == '__main__':
    ...
    if missing_files:
         ...
         # (Error: The following data files were not found:)
    ...
    openinterest_col_name = None # 不使用持仓量
    # openinterest_col_name = None # Don't use open interest
    ...
    if optimize:
        ...
        if best_result:
            ...
            print(header_format_str.format('ETF类型', 'EMA中', 'EMA长', 'BB周期', 'BB标差', 'ATR止损', '区间SL', '趋势风险%', '区间风险%', '夏普', '收益%', '回撤%', '得分'))
            # (Print header row)
            print('-' * total_width)

            all_scored_results.sort(key=lambda x: x.get('score', float('-inf')), reverse=True)
            ...
            for res_data in all_scored_results[:min(20, len(all_scored_results))]:
                ...
                print(row_format_str.format(
                    ...
                    trend_stop_loss_atr_mult=p_dict.get('trend_stop_loss_atr_mult', 0.0),
                    range_stop_loss_buffer=p_dict.get('range_stop_loss_buffer', 0.0),
                    risk_per_trade_trend=p_dict.get('risk_per_trade_trend', 0.0) * 100,
                    risk_per_trade_range=p_dict.get('risk_per_trade_range', 0.0) * 100,
            ...
            print(f"{'区间止损缓冲':<25}: {best_params_dict.get('range_stop_loss_buffer', 0.0):.4f}")
            print(f"{'趋势交易风险':<25}: {best_params_dict.get('risk_per_trade_trend', 0.0)*100:.2f}%")
            print(f"{'区间交易风险':<25}: {best_params_dict.get('risk_per_trade_range', 0.0)*100:.2f}%")
        ...
        if not optimize:
            try:
                print("\n尝试绘制图表...")
                plot_filename = 'backtest_plot_decoupled.png'
                ...
                if data_to_plot:
                    try:
                        cerebro.plot(style='candlestick', barup='red', bardown='green',
                                     ...
                                     plotdatanames=[data_to_plot], plotter='bokeh')
                        print(f"图表已使用 Bokeh 保存到 {plot_filename}")
                    except Exception as plot_err:
                        print(f"使用 Bokeh 绘图失败 ({plot_err})，尝试使用 matplotlib...")
                        cerebro.plot(style='candlestick', barup='red', bardown='green',
                                     iplot=False, volume=True, savefig=True, figfilename=plot_filename,
                                     plotdatanames=[data_to_plot])
                        print(f"图表已使用 Matplotlib 保存到 {plot_filename}")
                else:
                    print("没有数据可供绘制。")
            except ImportError:
                 print("\n无法绘制图表：缺少 'matplotlib' 或 'bokeh' 库。请使用 pip install matplotlib bokeh 安装。")
            except Exception as e:
                print(f"\n绘制图表时出错: {e}")
                print("请确保已安装绘图库 (matplotlib/bokeh) 且图形环境配置正确。")
