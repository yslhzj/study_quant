...
if __name__ == '__main__':
    ...
    if optimize:
        ...
        if not optimize:
            try:
                print("\n尝试绘制图表...")
                # 获取第一个数据源的名称用于绘图，如果存在
                # (Get the name of the first data feed for plotting, if it exists)
                data_to_plot = cerebro.datas[0]._name if cerebro.datas else None
                if data_to_plot:
                    # 调用 plot，不指定 plotter (默认为 matplotlib), 不保存文件 (移除 savefig 和 figfilename)
                    # iplot 默认为 True，会尝试显示图表
                    # (Call plot, without specifying plotter (defaults to matplotlib), do not save file (remove savefig and figfilename))
                    # iplot defaults to True, which will attempt to display the chart
                    cerebro.plot(style='candlestick', barup='red', bardown='green',
                                 volume=True, plotdatanames=[data_to_plot])
                    print("图表已生成，请查看绘图窗口。")
                    # (Chart generated, please check the plotting window.)
                else:
                    print("没有数据可供绘制。")
                    # (No data available for plotting.)
            except ImportError:
                print("\n无法绘制图表：缺少 'matplotlib' 库。请使用 pip install matplotlib 安装。")
                # (Unable to plot chart: 'matplotlib' library is missing. Please install using pip install matplotlib.)
            except Exception as e:
                print(f"\n绘制图表时出错: {e}")
                # (Error occurred while plotting chart: {e})
                print("请确保已安装绘图库 (matplotlib) 且图形环境配置正确。")
                # (Please ensure plotting library (matplotlib) is installed and graphics environment is configured correctly.)
