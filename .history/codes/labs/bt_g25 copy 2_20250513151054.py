    ...
    for strat_list in results:
        ...
        try:
            ...
            sharpe = sharpe_analysis.get(
                'sharperatio')  # Sharpe Ratio 可能为 None
...
if __name__ == '__main__':
    ...
    if optimize:
        ...
        if best_result:
            ...
            # 使用更新后的 row_format_str (风险百分比使用.2f)
            # (Use updated row_format_str (risk percentages use .2f))
            new_row_format_str = '{etf_type:<8} {ema_medium_period:<8.0f} {ema_long_period:<8.0f} {bbands_period:<8.0f} {bbands_devfactor:<8.1f} {trend_stop_loss_atr_mult:<8.1f} {range_stop_loss_buffer:<8.3f} {risk_per_trade_trend:<10.2f} {risk_per_trade_range:<10.2f} {sharpe:<10.4f} {return_val:<10.2f} {drawdown_val:<10.2f} {score:<10.4f}'
            ...
            for res_data in all_scored_results[:min(20, len(all_scored_results))]:
                ...
                print(new_row_format_str.format(
            ...
            print(f'\n{(" 最优参数组合 "):=^50}')
            best_p_dict = best_result.get('params_dict', {})

            print(f"\\n  --- 参数 ---")
            param_labels = {
                'etf_type': 'ETF类型',
                'ema_medium_period': 'EMA中期',
                'ema_long_period': 'EMA长期',
                'bbands_period': '布林带周期',
                'bbands_devfactor': '布林带标准差',
                'trend_stop_loss_atr_mult': '趋势止损ATR倍数',
                'range_stop_loss_buffer': '区间止损缓冲',
                'risk_per_trade_trend': '趋势交易风险',
                'risk_per_trade_range': '区间交易风险'
            }
            param_formats = {
                'ema_medium_period': "{:.0f}",
                'ema_long_period': "{:.0f}",
                'bbands_period': "{:.0f}",
                'bbands_devfactor': "{:.1f}",
                'trend_stop_loss_atr_mult': "{:.1f}",
                'range_stop_loss_buffer': "{:.3f}",
                'risk_per_trade_trend': "{:.2f}%",
                'risk_per_trade_range': "{:.2f}%"
            }

            label_width = 25
            for key, label in param_labels.items():
                val = best_p_dict.get(key, 'N/A')
                val_to_format = val
                if key in ['risk_per_trade_trend', 'risk_per_trade_range'] and isinstance(val, (int, float)):
                    val_to_format = val * 100

                formatted_val = param_formats.get(key, "{}").format(
                    val_to_format) if val != 'N/A' else 'N/A'
                print(f"  {label:<{label_width-4}}: {formatted_val}")
            ...
            for key, label in metric_labels_best.items():
                val = best_result.get(key, 0.0)
                val_to_format = val
                if key in ['return', 'drawdown'] and isinstance(val, (int, float)):
                    val_to_format = val * 100

                formatted_val = metric_formats_best.get(
                    key, "{}").format(val_to_format)
                print(f"  {label:<{label_width-4}}: {formatted_val}")
            print('=' * 50)
        else:
            print("\n错误：未能确定最优策略或处理结果时出错。")
