...
class AShareETFSizer(bt.Sizer):
    ...
    params = (
        ...
        ('trend_stop_loss_atr_mult_param_name', 'trend_stop_loss_atr_mult'),
            # 趋势止损ATR倍数参数的名称
            # (告诉Sizer去哪里找策略中趋势止损的ATR倍数参数)
        
        ('range_stop_loss_buffer_param_name', 'range_stop_loss_buffer'),
            # 区间止损缓冲参数的名称
            # (告诉Sizer去哪里找策略中区间止损的缓冲参数)
        
        ('atr_indicator_name_prefix', 'atr_')
            # ATR指标名称前缀，默认为'atr_'
            # (帮助Sizer找到策略中存储的ATR指标)
    )

    def _getsizing(self, comminfo, cash, data, isbuy):
        # 计算头寸大小的核心方法，由backtrader在需要确定买卖数量时调用
        # (这是决定买多少股票的核心函数，每次交易前都会被调用)
        
        if not isbuy:
            # 如果不是买入操作，则返回0
            # (这个Sizer只管买不管卖，所以遇到卖单就直接返回0)
            return 0
        ...
        current_etf_type = getattr(strategy.params, self.p.etf_type_param_name, 'trend')
            # 从策略参数中获取当前ETF类型，默认为'trend'
            # (去策略设置里看看这只ETF是什么类型的，没设置就默认是趋势型)
        ...
        if stop_loss_price_ref >= entry_price_ref:
            # 如果止损价不低于买入价，记录日志并返回0
            # (如果算出来的止损价比买入价还高，这肯定不对)
            strategy.log(f"Sizer: Stop loss {stop_loss_price_ref:.2f} not below entry {entry_price_ref:.2f} for {d_name}. Cannot size.", data=data)
            return 0
