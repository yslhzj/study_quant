...
import time
# 导入 `time` 库，提供时间相关的功能，例如获取当前时间、程序暂停执行等。
# (引入一个叫做 `time` 的工具箱，它可以帮我们获取当前时间，或者让程序等一会儿再继续运行。)
import itertools
# 导入 `itertools` 库，提供用于创建高效迭代器的函数。
# (引入一个叫做 `itertools` 的工具箱，它里面有很多巧妙的方法来处理一串接一串的数据，让处理过程更省力。)
